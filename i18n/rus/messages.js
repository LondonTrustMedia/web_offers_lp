;var json_locale_data = {
   "messages": {
      "": {
         "Project-Id-Version": " pia-lps\nLanguage-Team: Russian\nReport-Msgid-Bugs-To: \nPO-Revision-Date: 2020-05-20 14:02\nLanguage: ru_RU\nMIME-Version: 1.0\nContent-Type: text/plain; charset=UTF-8\nContent-Transfer-Encoding: 8bit\nPOT-Creation-Date: 2020-05-13 10:52+0000\nPlural-Forms: nplurals=4; plural=((n%10==1 && n%100!=11) ? 0 : ((n%10 >= 2 && n%10 <=4 && (n%100 < 12 || n%100 > 14)) ? 1 : ((n%10 == 0 || (n%10 >= 5 && n%10 <=9)) || (n%100 >= 11 && n%100 <= 14)) ? 2 : 3));\nX-Crowdin-Project: pia-lps\nX-Crowdin-Language: ru\nX-Crowdin-File: messages.pot\n"
      },
      "Afghanistan": [
         null,
         "Афганистан"
      ],
      "Åland Islands": [
         null,
         "Аландские острова"
      ],
      "Albania": [
         null,
         "Албания"
      ],
      "Algeria": [
         null,
         "Алжир"
      ],
      "American Samoa": [
         null,
         "Американское Самоа"
      ],
      "Andorra": [
         null,
         "Андорра"
      ],
      "Angola": [
         null,
         "Ангола"
      ],
      "Anguilla": [
         null,
         "Ангилья"
      ],
      "Antarctica": [
         null,
         "Антарктика"
      ],
      "Antigua and Barbuda": [
         null,
         "Антигуа и Барбуда"
      ],
      "Argentina": [
         null,
         "Аргентина"
      ],
      "Armenia": [
         null,
         "Армения"
      ],
      "Aruba": [
         null,
         "Аруба"
      ],
      "Australia": [
         null,
         "Австралия"
      ],
      "Austria": [
         null,
         "Австрия"
      ],
      "Azerbaijan": [
         null,
         "Азербайджан"
      ],
      "Bahamas": [
         null,
         "Багамы"
      ],
      "Bahrain": [
         null,
         "Бахрейн"
      ],
      "Bangladesh": [
         null,
         "Бангладеш"
      ],
      "Barbados": [
         null,
         "Барбадос"
      ],
      "Belarus": [
         null,
         "Беларусь"
      ],
      "Belgium": [
         null,
         "Бельгия"
      ],
      "Belize": [
         null,
         "Белиз"
      ],
      "Benin": [
         null,
         "Бенин"
      ],
      "Bermuda": [
         null,
         "Бермуды"
      ],
      "Bhutan": [
         null,
         "Бутан"
      ],
      "Bolivia": [
         null,
         "Боливия"
      ],
      "Caribbean Netherlands": [
         null,
         "Карибские Нидерланды"
      ],
      "Bosnia and Herzegovina": [
         null,
         "Босния и Герцеговина"
      ],
      "Botswana": [
         null,
         "Ботсвана"
      ],
      "Bouvet Island": [
         null,
         "Остров Буве"
      ],
      "Brazil": [
         null,
         "Бразилия"
      ],
      "British Indian Ocean Territory": [
         null,
         "Британская территория Индийского океана"
      ],
      "Brunei Darussalam": [
         null,
         "Бруней Даруссалам"
      ],
      "Bulgaria": [
         null,
         "Болгария"
      ],
      "Burkina Faso": [
         null,
         "Буркина-Фасо"
      ],
      "Burundi": [
         null,
         "Бурунди"
      ],
      "Cabo Verde": [
         null,
         "Кабо-Верде"
      ],
      "Cambodia": [
         null,
         "Камбоджа"
      ],
      "Cameroon": [
         null,
         "Камерун"
      ],
      "Canada": [
         null,
         "Канада"
      ],
      "Cayman Islands": [
         null,
         "Каймановы острова"
      ],
      "Central African Republic": [
         null,
         "Центрально-Африканская Республика"
      ],
      "Chad": [
         null,
         "Чад"
      ],
      "Chile": [
         null,
         "Чили"
      ],
      "China": [
         null,
         "Китай"
      ],
      "Christmas Island": [
         null,
         "Остров Рождества"
      ],
      "Cocos (Keeling) Islands": [
         null,
         "Кокосовые (Килинг) острова"
      ],
      "Colombia": [
         null,
         "Колумбия"
      ],
      "Comoros": [
         null,
         "Коморские острова"
      ],
      "Congo": [
         null,
         "Конго"
      ],
      "Democratic Republic of the Congo": [
         null,
         "Демократическая Республика Конго"
      ],
      "Cook Islands": [
         null,
         "Острова Кука"
      ],
      "Costa Rica": [
         null,
         "Коста-Рика"
      ],
      "Côte d'Ivoire": [
         null,
         "Кот-д’Ивуар"
      ],
      "Croatia": [
         null,
         "Хорватия"
      ],
      "Cuba": [
         null,
         "Куба"
      ],
      "Curaçao": [
         null,
         "Кюрасао"
      ],
      "Cyprus": [
         null,
         "Кипр"
      ],
      "Czechia": [
         null,
         "Чехия"
      ],
      "Denmark": [
         null,
         "Дания"
      ],
      "Djibouti": [
         null,
         "Джибути"
      ],
      "Dominica": [
         null,
         "Доминика"
      ],
      "Dominican Republic": [
         null,
         "Доминиканская Республика"
      ],
      "Ecuador": [
         null,
         "Эквадор"
      ],
      "Egypt": [
         null,
         "Египет"
      ],
      "El Salvador": [
         null,
         "Сальвадор"
      ],
      "Equatorial Guinea": [
         null,
         "Экваториальная Гвинея"
      ],
      "Eritrea": [
         null,
         "Эритрея"
      ],
      "Estonia": [
         null,
         "Эстония"
      ],
      "Eswatini": [
         null,
         "Эсватини"
      ],
      "Ethiopia": [
         null,
         "Эфиопия"
      ],
      "Falkland Islands (Malvinas)": [
         null,
         "Фолклендские (Мальвинские) острова"
      ],
      "Faroe Islands": [
         null,
         "Фарерские острова"
      ],
      "Fiji": [
         null,
         "Фиджи"
      ],
      "Finland": [
         null,
         "Финляндия"
      ],
      "France": [
         null,
         "Франция"
      ],
      "French Guiana": [
         null,
         "Французская Гвиана"
      ],
      "French Polynesia": [
         null,
         "Французская Полинезия"
      ],
      "French Southern Territories": [
         null,
         "Французские Южные территории"
      ],
      "Gabon": [
         null,
         "Габон"
      ],
      "Gambia": [
         null,
         "Гамбия"
      ],
      "Georgia": [
         null,
         "Грузия"
      ],
      "Germany": [
         null,
         "Германия"
      ],
      "Ghana": [
         null,
         "Гана"
      ],
      "Gibraltar": [
         null,
         "Гибралтар"
      ],
      "Greece": [
         null,
         "Греция"
      ],
      "Greenland": [
         null,
         "Гренландия"
      ],
      "Grenada": [
         null,
         "Гренада"
      ],
      "Guadeloupe": [
         null,
         "Гваделупа"
      ],
      "Guam": [
         null,
         "Гуам"
      ],
      "Guatemala": [
         null,
         "Гватемала"
      ],
      "Guernsey": [
         null,
         "Гернси"
      ],
      "Guinea": [
         null,
         "Гвинея"
      ],
      "Guinea-Bissau": [
         null,
         "Гвинея-Бисау"
      ],
      "Guyana": [
         null,
         "Гайана"
      ],
      "Haiti": [
         null,
         "Гаити"
      ],
      "Heard Island and McDonald Islands": [
         null,
         "Остров Херд и острова Макдональд"
      ],
      "Holy See": [
         null,
         "Святой Престол"
      ],
      "Honduras": [
         null,
         "Гондурас"
      ],
      "Hong Kong": [
         null,
         "Гонконг"
      ],
      "Hungary": [
         null,
         "Венгрия"
      ],
      "Iceland": [
         null,
         "Исландия"
      ],
      "India": [
         null,
         "Индия"
      ],
      "Indonesia": [
         null,
         "Индонезия"
      ],
      "Iran": [
         null,
         "Иран"
      ],
      "Iraq": [
         null,
         "Ирак"
      ],
      "Ireland": [
         null,
         "Ирландия"
      ],
      "Isle of Man": [
         null,
         "Остров Мэн"
      ],
      "Israel": [
         null,
         "Израиль"
      ],
      "Italy": [
         null,
         "Италия"
      ],
      "Jamaica": [
         null,
         "Ямайка"
      ],
      "Japan": [
         null,
         "Япония"
      ],
      "Jersey": [
         null,
         "Джерси"
      ],
      "Jordan": [
         null,
         "Иордания"
      ],
      "Kazakhstan": [
         null,
         "Казахстан"
      ],
      "Kenya": [
         null,
         "Кения"
      ],
      "Kiribati": [
         null,
         "Кирибати"
      ],
      "North Korea": [
         null,
         "Северная Корея"
      ],
      "South Korea": [
         null,
         "Южная Корея"
      ],
      "Kuwait": [
         null,
         "Кувейт"
      ],
      "Kyrgyzstan": [
         null,
         "Киргизия"
      ],
      "Laos": [
         null,
         "Лаос"
      ],
      "Latvia": [
         null,
         "Латвия"
      ],
      "Lebanon": [
         null,
         "Ливан"
      ],
      "Lesotho": [
         null,
         "Лесото"
      ],
      "Liberia": [
         null,
         "Либерия"
      ],
      "Libya": [
         null,
         "Ливия"
      ],
      "Liechtenstein": [
         null,
         "Лихтенштейн"
      ],
      "Lithuania": [
         null,
         "Литва"
      ],
      "Luxembourg": [
         null,
         "Люксембург"
      ],
      "Macao": [
         null,
         "Макао"
      ],
      "Madagascar": [
         null,
         "Мадагаскар"
      ],
      "Malawi": [
         null,
         "Малави"
      ],
      "Malaysia": [
         null,
         "Малайзия"
      ],
      "Maldives": [
         null,
         "Мальдивы"
      ],
      "Mali": [
         null,
         "Мали"
      ],
      "Malta": [
         null,
         "Мальта"
      ],
      "Marshall Islands": [
         null,
         "Маршалловы острова"
      ],
      "Martinique": [
         null,
         "Мартиника"
      ],
      "Mauritania": [
         null,
         "Мавритания"
      ],
      "Mauritius": [
         null,
         "Маврикий"
      ],
      "Mayotte": [
         null,
         "Майотт"
      ],
      "Mexico": [
         null,
         "Мексика"
      ],
      "Federated States of Micronesia": [
         null,
         "Федеративные Штаты Микронезии"
      ],
      "Moldova": [
         null,
         "Молдова"
      ],
      "Monaco": [
         null,
         "Монако"
      ],
      "Mongolia": [
         null,
         "Монголия"
      ],
      "Montenegro": [
         null,
         "Черногория"
      ],
      "Montserrat": [
         null,
         "Монтсеррат"
      ],
      "Morocco": [
         null,
         "Марокко"
      ],
      "Mozambique": [
         null,
         "Мозамбик"
      ],
      "Myanmar": [
         null,
         "Мьянма"
      ],
      "Namibia": [
         null,
         "Намибия"
      ],
      "Nauru": [
         null,
         "Науру"
      ],
      "Nepal": [
         null,
         "Непал"
      ],
      "Netherlands": [
         null,
         "Нидерланды"
      ],
      "New Caledonia": [
         null,
         "Новая Каледония"
      ],
      "New Zealand": [
         null,
         "Новая Зеландия"
      ],
      "Nicaragua": [
         null,
         "Никарагуа"
      ],
      "Niger": [
         null,
         "Нигер"
      ],
      "Nigeria": [
         null,
         "Нигерия"
      ],
      "Niue": [
         null,
         "Ниуэ"
      ],
      "Norfolk Island": [
         null,
         "Остров Норфолк"
      ],
      "North Macedonia": [
         null,
         "Северная Македония"
      ],
      "Northern Mariana Islands": [
         null,
         "Северные Марианские острова"
      ],
      "Norway": [
         null,
         "Норвегия"
      ],
      "Oman": [
         null,
         "Оман"
      ],
      "Pakistan": [
         null,
         "Пакистан"
      ],
      "Palau": [
         null,
         "Палау"
      ],
      "Panama": [
         null,
         "Панама"
      ],
      "Papua New Guinea": [
         null,
         "Папуа — Новая Гвинея"
      ],
      "Paraguay": [
         null,
         "Парагвай"
      ],
      "Peru": [
         null,
         "Перу"
      ],
      "Philippines": [
         null,
         "Филиппины"
      ],
      "Pitcairn": [
         null,
         "Питкэрн"
      ],
      "Poland": [
         null,
         "Польша"
      ],
      "Portugal": [
         null,
         "Португалия"
      ],
      "Puerto Rico": [
         null,
         "Пуэрто-Рико"
      ],
      "Qatar": [
         null,
         "Катар"
      ],
      "Réunion": [
         null,
         "Реюньон"
      ],
      "Romania": [
         null,
         "Румыния"
      ],
      "Russia": [
         null,
         "Россия"
      ],
      "Rwanda": [
         null,
         "Руанда"
      ],
      "Saint Barthélemy": [
         null,
         "Сен-Бартелеми"
      ],
      "Saint Helena, Ascension and Tristan da Cunha": [
         null,
         "Острова Святой Елены, Вознесения и Тристан-да-Кунья"
      ],
      "Saint Kitts and Nevis": [
         null,
         "Сент-Китс и Невис"
      ],
      "Saint Lucia": [
         null,
         "Сент-Люсия"
      ],
      "Saint Martin (French part)": [
         null,
         "Синт-Мартен (владение Франции)"
      ],
      "Saint Pierre and Miquelon": [
         null,
         "Сен-Пьер и Микелон"
      ],
      "Saint Vincent and the Grenadines": [
         null,
         "Сент-Винсент и Гренадин"
      ],
      "Samoa": [
         null,
         "Самоа"
      ],
      "San Marino": [
         null,
         "Сан-Марино"
      ],
      "Sao Tome and Principe": [
         null,
         "Сан-Томе и Принсипи"
      ],
      "Saudi Arabia": [
         null,
         "Саудовская Аравия"
      ],
      "Senegal": [
         null,
         "Сенегал"
      ],
      "Serbia": [
         null,
         "Сербия"
      ],
      "Seychelles": [
         null,
         "Сейшельские острова"
      ],
      "Sierra Leone": [
         null,
         "Сьерра-Леоне"
      ],
      "Singapore": [
         null,
         "Сингапур"
      ],
      "Sint Maarten (Dutch part)": [
         null,
         "Синт-Мартен (владение Нидерландов)"
      ],
      "Slovakia": [
         null,
         "Словакия"
      ],
      "Slovenia": [
         null,
         "Словения"
      ],
      "Solomon Islands": [
         null,
         "Соломоновы острова"
      ],
      "Somalia": [
         null,
         "Сомали"
      ],
      "South Africa": [
         null,
         "Южная Африка"
      ],
      "South Georgia and the South Sandwich Islands": [
         null,
         "Южная Георгия и Южные Сандвичевы острова"
      ],
      "South Sudan": [
         null,
         "Южный Судан"
      ],
      "Spain": [
         null,
         "Испания"
      ],
      "Sri Lanka": [
         null,
         "Шри-Ланка"
      ],
      "Sudan": [
         null,
         "Судан"
      ],
      "Suriname": [
         null,
         "Суринам"
      ],
      "Svalbard and Jan Mayen": [
         null,
         "Шпицберген и Ян Майен"
      ],
      "Sweden": [
         null,
         "Швеция"
      ],
      "Switzerland": [
         null,
         "Швейцария"
      ],
      "Syria": [
         null,
         "Сирия"
      ],
      "Taiwan, Province of China": [
         null,
         "Тайвань, провинция Китая"
      ],
      "Tajikistan": [
         null,
         "Таджикистан"
      ],
      "Tanzania": [
         null,
         "Танзания"
      ],
      "Thailand": [
         null,
         "Таиланд"
      ],
      "Timor-Leste": [
         null,
         "Восточный Тимор"
      ],
      "Togo": [
         null,
         "Того"
      ],
      "Tokelau": [
         null,
         "Токелау"
      ],
      "Tonga": [
         null,
         "Тонга"
      ],
      "Trinidad and Tobago": [
         null,
         "Тринидад и Тобаго"
      ],
      "Tunisia": [
         null,
         "Тунис"
      ],
      "Turkey": [
         null,
         "Турция"
      ],
      "Turkmenistan": [
         null,
         "Туркменистан"
      ],
      "Turks and Caicos Islands": [
         null,
         "О-ва Теркс и Кайкос"
      ],
      "Tuvalu": [
         null,
         "Тувалу"
      ],
      "Uganda": [
         null,
         "Уганда"
      ],
      "Ukraine": [
         null,
         "Украина"
      ],
      "the United Arab Emirates": [
         null,
         "Объединенные Арабские Эмираты"
      ],
      "the United Kingdom": [
         null,
         "Великобритания"
      ],
      "the United States": [
         null,
         "США"
      ],
      "the United States Minor Outlying Islands": [
         null,
         "Малые Тихоокеанские отдаленные острова Соединенных Штатов"
      ],
      "Uruguay": [
         null,
         "Уругвай"
      ],
      "Uzbekistan": [
         null,
         "Узбекистан"
      ],
      "Vanuatu": [
         null,
         "Вануату"
      ],
      "Venezuela": [
         null,
         "Венесуэла"
      ],
      "Viet Nam": [
         null,
         "Вьетнам"
      ],
      "Virgin Islands (British)": [
         null,
         "Виргинские острова (Британские)"
      ],
      "Virgin Islands (U.S.)": [
         null,
         "Виргинские острова (США)"
      ],
      "Wallis and Futuna": [
         null,
         "Уоллис и Футуна"
      ],
      "Western Sahara": [
         null,
         "Западная Сахара"
      ],
      "Yemen": [
         null,
         "Йемен"
      ],
      "Zambia": [
         null,
         "Замбия"
      ],
      "Zimbabwe": [
         null,
         "Зимбабве"
      ],
      "Russian Rubles": [
         null,
         "Российские рубли"
      ],
      "Indian Rupee": [
         null,
         ""
      ],
      "Brazilian Reals": [
         null,
         ""
      ],
      "Japanese Yen": [
         null,
         ""
      ],
      "Australian Dollars": [
         null,
         ""
      ],
      "Swiss Francs": [
         null,
         "Швейцарский франк"
      ],
      "Euros": [
         null,
         "Евро"
      ],
      "US Dollars": [
         null,
         "Доллары США"
      ],
      "Join now": [
         null,
         "Присоединиться сейчас"
      ],
      "Great": [
         null,
         "Отлично"
      ],
      "Based on %s reviews": [
         null,
         "На основе %s отзывов"
      ],
      "Read full review": [
         null,
         "Читать полный обзор"
      ],
      "Trusted By Companies Including:": [
         null,
         "Проверено компаниями, в том числе:"
      ],
      "World-class VPN service:": [
         null,
         "VPN-сервис мирового класса:"
      ],
      "Protect your online privacy and identity": [
         null,
         "Защитите свою конфиденциальность и личные данные в Интернете"
      ],
      "Browse anonymously with a private IP address and enable strong encryption to protect yourself from your ISP and public Wi-Fi networks.": [
         null,
         "Посещайте сайты анонимно, используя приватный IP-адрес, и используйте стойкое шифрование для защиты от слежки провайдера и атак в общедоступных сетях Wi-Fi."
      ],
      "Unblock Streaming": [
         null,
         "Разблокировка стриминга"
      ],
      "Get access to tons of new content on platforms like Netflix, Disney Plus, Amazon Prime Video, HBO GO, and more.": [
         null,
         "Получите доступ к тысячам наименований нового контента на таких платформах, как Netflix, Disney Plus, Amazon Prime Video, HBO GO и многих других."
      ],
      "The World's Leading VPN": [
         null,
         "Лидирующий мировой VPN-сервис"
      ],
      "Private Internet Access provides the highest speeds and most encryption. So, get a perfect streaming experience without any buffering or slow loading times.": [
         null,
         "Private Internet Access обеспечивает высочайшую скорость и стойкое шифрование. оцените превосходное качество стриминга без буферизации и долгого ожидания загрузки."
      ],
      "Monthly": [
         null,
         "Ежемесячно"
      ],
      "Months": [
         null,
         "Месяцы"
      ],
      "Yearly": [
         null,
         "Ежегодно"
      ],
      "Years": [
         null,
         "Годы"
      ],
      "Best value": [
         null,
         "Лучшее предложение"
      ],
      "Save <b>%s%</b>": [
         null,
         "Сэкономьте <b>%s%</b>"
      ],
      "/mo": [
         null,
         " в месяц"
      ],
      "<del>%s</del> %s per month": [
         null,
         "<del>%s</del> %s в месяц"
      ],
      "<del>%s</del> %s per %s months": [
         null,
         "<del>%s</del> %s за %s мес."
      ],
      "<del>%s</del> %s per year": [
         null,
         "<del>%s</del> %s в год"
      ],
      "<del>%s</del>%s per %s years": [
         null,
         "<del>%s</del>%s за %s гг"
      ],
      "Sign up now": [
         null,
         "Зарегистрируйтесь сейчас"
      ],
      "Best<br>value": [
         null,
         "Лучшее<br>предложение"
      ],
      "+%s extra<br>months free": [
         null,
         "+ %s доп.<br>мес. бесплатно"
      ],
      "All amounts are shown in %s, and any discounts reflect a reduction based on the current monthly service pricing at %s per month": [
         null,
         "Все суммы указаны в %s, а все скидки означают снижение базовой текущей цены месячного обслуживания %s в месяц"
      ],
      "Why choose Private Internet Access?": [
         null,
         "Почему стоит выбрать Private Internet Access?"
      ],
      "Total Privacy": [
         null,
         "Полная приватность"
      ],
      "Private Internet Access never keeps any records of users' online activities. You can rest assured knowing you have complete privacy, and nobody knows what you're doing online.": [
         null,
         "Private Internet Access никогда не ведет никакие записи онлайн-действий пользователей. Вы можете быть уверены, что ваша приватность защищена, и никто не знает, что вы делаете онлайн."
      ],
      "Easy to use": [
         null,
         "Простота использования"
      ],
      "No matter your technical skills Private Internet Access is one of the easiest VPN apps out there. All it takes to connect is just 1 click or tap and your data will be encrypted instantly.": [
         null,
         "Независимо от уровня ваших технических знаний Private Internet Access является одним из самых простых в использовании приложений. Для подключения достаточно одного клика или нажатия, после которого все ваши данные будут мгновенно зашифрованы."
      ],
      "Secure VPN Protocols": [
         null,
         "Безопасные VPN-протоколы"
      ],
      "Our app is powered by the latest and most advanced protocols like: Wireguard, PPTP, OpenVPN and L2TP/IPSec.": [
         null,
         "Наше приложение поддерживает новейшие и самые совершенные протоколы, такие как Wireguard, PPTP, OpenVPN и L2TP/IPSec."
      ],
      "%s devices simultaneously": [
         null,
         "Одновременное подключение %s устройств"
      ],
      "Keep all your devices protected. With just 1 Private Internet Access VPN subscription you can connect on up to %s devices at the same time.": [
         null,
         "Обеспечьте защиту всех своих устройств. Всего одна подписка Private Internet Access VPN позволяет одновременно подключать до %s устройств."
      ],
      "Our readers made it abundantly clear that Private Internet Access should be your first stop for protecting your private browsing data.": [
         null,
         "Наши читатели однозначно заявили, что Private Internet Access является лучшим выбором для защиты ваших приватных данных онлайн-активности."
      ],
      "Private Internet Access | Anonymous VPN Service Provider": [
         null,
         "Private Internet Access | Поставщик анонимного VPN-сервиса"
      ],
      "As recommended by": [
         null,
         "Рекомендовано"
      ],
      "Limited Time Only! <span class='color_pia_green'>Act Now</span>": [
         null,
         "Время ограничено! <span class='color_pia_green'>Действуйте сейчас</span>"
      ],
      "%s<br><span>day</span>": [
         null,
         "%s<br><span>д.</span>"
      ],
      "money back guarantee": [
         null,
         "гарантия возврата денег"
      ],
      "Remember, you're not risking anything.": [
         null,
         "Помните, вы ничем не рискуете."
      ],
      "If you’re not 100% satisfied with <b>Private Internet Access</b>,<br>you have %s full days to ask for a refund and we’ll give you your money back.": [
         null,
         "Если вы не удовлетворены <b>Private Internet Access</b> на 100 %,<br>у вас есть %s полных дней, чтобы запросить возврат денег, и мы вернем вам уплаченную сумму."
      ],
      "FREE": [
         null,
         "БЕСПЛАТНО"
      ],
      "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 46.0271 120.4594)' font-size='17px'>EXTRA MONTHS</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>FREE": [
         null,
         "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text font-size='25px' transform='matrix(1, 0.2341, -0.2341, 1, 55, 135)'>МЕСЯЦА</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>БЕСПЛАТНО"
      ],
      "Your IP:": [
         null,
         "Ваш IP-адрес:"
      ],
      "Your Location:": [
         null,
         "Ваше местоположение:"
      ],
      "Your ISP:": [
         null,
         "Ваш интернет-провайдер:"
      ],
      "Your Status:": [
         null,
         "Ваш статус:"
      ],
      "You are not protected": [
         null,
         "Вы не защищены"
      ],
      "<p><b class='text-uppercase'>\"Private Internet Access</b></p><p>out-performs and out-features the competition\"</p>": [
         null,
         "<p><b class='text-uppercase'>\"Private Internet Access</b></p><p>лучше и качественнее любых конкурентов\" </p>"
      ],
      "Connect to servers from all over the world": [
         null,
         "Доступ к серверам в разных странах"
      ],
      "Servers": [
         null,
         "Серверы"
      ],
      "Countries": [
         null,
         "Страны"
      ],
      "for ONLY": [
         null,
         "ВСЕГО за"
      ],
      "/month": [
         null,
         " в месяц"
      ],
      "Proud Sponsor Of": [
         null,
         "С гордостью спонсируем"
      ],
      "Compatible with all popular desktop and mobile operating systems": [
         null,
         "Совместимость со всеми распространенными операционными системами для ПК и мобильных устройств"
      ],
      "For a Limited Time Only<br>Grab our": [
         null,
         "Время предложения ограничено<br>Воспользуйтесь сейчас"
      ],
      "DISCOUNT": [
         null,
         "СКИДКА"
      ],
      "Experience": [
         null,
         "Оцените"
      ],
      "Total Online Freedom": [
         null,
         "Полная онлайн-свобода"
      ],
      "Bypass censorship and unblock sites and content you don't have access to": [
         null,
         "Обходите цензуру и блокировку сайтов и контента, к которым у вас нет доступа"
      ],
      "Secure Protection on Public Wi-Fi": [
         null,
         "Надежная защита в общедоступных сетях Wi-Fi"
      ],
      "Keep your data safe and private while connecting to any poorly secured network": [
         null,
         "Обеспечьте приватность и защиту своих данных при подключении к сетям с низким уровнем защиты"
      ],
      "Unlimited Bandwidth & Fast Download Speeds": [
         null,
         "Неограниченная пропускная способность и высокая скорость загрузки"
      ],
      "Keep your downloads private and experience lightning fast speeds for P2P torrenting": [
         null,
         "Обеспечьте приватность своих загрузок и оцените высочайшую скорость при загрузке торрентов"
      ],
      "Ad blocking": [
         null,
         "Блокировка рекламы"
      ],
      "With Private Internet Access you can block ads, trackers, and malware": [
         null,
         "С помощью Private Internet Access можно блокировать рекламу, трекеры и вредоносное ПО"
      ],
      "Get Private Internet Access VPN": [
         null,
         "Получите Private Internet Access VPN"
      ],
      "For a Limited Time Only": [
         null,
         "Действует ограниченное время"
      ],
      "OOPS!": [
         null,
         "ОЙ!"
      ],
      "It looks like the page you were looking for isn't here anymore. Feel free to visit our homepage by clicking the button below.": [
         null,
         "Похоже, странница, которую вы ищете, больше не существует. Посетите нашу главную страницу, нажав кнопку ниже."
      ],
      "Take Me To The Homepage": [
         null,
         "Перейти на главную страницу"
      ],
      "<small>Discover all the</small><br class=\"\"><b><span>streaming content</span></b><br>you’ve been missing out on": [
         null,
         "<small>Исследуйте весь</small><br class=\"\"><b><span>стриминговый контент,</span></b><br>которого вы были лишены"
      ],
      "Unblock restricted content libraries easily<br> with Private Internet Access": [
         null,
         "С легкостью получите доступ к заблокированным медиатекам контента с помощью<br> Private Internet Access"
      ],
      "Try it risk free! <i class='fas fa-award'></i><span class='text-uppercase'> %s-day money back guarantee</span>": [
         null,
         "Попробуйте без риска! <i class='fas fa-award'></i><span class='text-uppercase'> %s-дневная гарантия возврата денег</span>"
      ],
      "Try it risk free!<br><span class='text-uppercase'>%s-day money back guarantee</span>": [
         null,
         "Попробуйте без риска!<br><span class='text-uppercase'>%s-дневная гарантия возврата денег</span>"
      ],
      "Private Internet Access VPN for %s% OFF": [
         null,
         "Private Internet Access VPN со СКИДКОЙ %s%"
      ],
      "Discover Limitless Streaming Content": [
         null,
         "Наслаждайтесь стриминговым контентом без ограничений"
      ],
      "Online streaming services and websites are becoming more popular every day all over the world. But sometimes geo-restrictions can limit the content you have access to. Don’t miss out on video streaming opportunities because of your location. <br><b class='mt-1 d-inline-block'>Sign up for Private Internet Access today!</b>": [
         null,
         "Стриминговые сервисы и сайты с каждым днем становятся все популярнее по всему миру. Но иногда региональные блокировки ограничивают доступный вам контент. Не упускайте возможности посмотреть что-то из за вашего местоположения. <br><b class='mt-1 d-inline-block'>Зарегистрируйтесь как пользователь Private Internet Access сегодня!</b>"
      ],
      "Unblock content on:": [
         null,
         "Разблокируйте контент в:"
      ],
      "Sign up for Private Internet Access today!": [
         null,
         "Зарегистрируйтесь как пользователь Private Internet Access сегодня!"
      ],
      "A must have for your streaming needs": [
         null,
         "Самое необходимое для просмотра стриминга"
      ],
      "Unlimited bandwidth for streaming": [
         null,
         "Неограниченная пропускная способность для стриминга"
      ],
      "Private Internet Access has no logs, never records online activity and does not throttle users.": [
         null,
         "В Private Internet Access не ведутся логи, не фиксируется онлайн-активность и не ограничивается пропускная способность для пользователей."
      ],
      "One-click solution to override geographical restrictions": [
         null,
         "Быстрое решение для обхода региональных ограничений"
      ],
      "Once you have chosen your virtual location, you can connect to it using Private Internet Access with just one click.": [
         null,
         "Когда вы выбираете свое виртуальное местоположение, вы можете подключиться к нему в Private Internet Access одним нажатием."
      ],
      "Stream on multiple devices": [
         null,
         "Стриминг на нескольких устройствах"
      ],
      "One Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your entire family!": [
         null,
         "Одну подписку Private Internet Access можно одновременно использовать на нескольких устройствах (до %s). Используйте Private Internet Access совместно с членами семьи!"
      ],
      "Four easy steps to stream with Private Internet Access": [
         null,
         "Четыре простых шага, чтобы смотреть стриминг с Private Internet Access"
      ],
      "Start streaming with Private Internet Access in minutes": [
         null,
         "Начните смотреть стриминг с Private Internet Access за пару минут"
      ],
      "<b>Subscribe</b><br>to Private Internet Access": [
         null,
         "<b>Приобретите подписку</b><br>Private Internet Access"
      ],
      "<b>Download</b><br>Private Internet Access": [
         null,
         "<b>Загрузите приложение</b><br>Private Internet Access"
      ],
      "<b>Login</b><br>on the Private Internet Access client with your username and password": [
         null,
         "<b>Выполните вход</b><br>в клиенте Private Internet Access, используя свое имя пользователя и пароль"
      ],
      "<b>Turn on</b><br>Private Internet Access and connect to the location of your choice": [
         null,
         "<b>Включите</b><br>Private Internet Access и подключитесь к выбранному местоположению"
      ],
      "You’re ready to start streaming with <span>Private Internet Access</span>": [
         null,
         "Все готово к просмотру стриминга с <span>Private Internet Access</span>"
      ],
      "Private Internet Access Streaming FAQ": [
         null,
         "Ответы на вопросы о Private Internet Access"
      ],
      "Is it safe to stream with Private Internet Access?": [
         null,
         "Безопасно ли смотреть стриминг с Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to stream without Private Internet Access?": [
         null,
         "Да. Private Internet Access предоставляет службу VPN с серверами в %s местоположениях, расположенных в %s странах, включая США, Великобританию, Японию и другие. Более правильный вопрос: безопасно ли смотреть стриминг без Private Internet Access?"
      ],
      "Do you have a refund policy?": [
         null,
         "Предусмотрен ли у вас возврат денег?"
      ],
      "Private Internet Access has a no questions asked %s day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "У Private Internet Access действует гарантия возврата денег в течение %s дней без лишних вопросов. Приобретайте подписку Private Internet Access сегодня, чтобы сразу начать просмотр стриминга!"
      ],
      "Will my IP address be exposed to streaming services or websites?": [
         null,
         "Будет ли мой IP-адрес виден стриминговым сервисам и сайтам?"
      ],
      "By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Streaming services and websites will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "При использовании Private Internet Access ваш трафик шифруется в VPN-тоннеле и попадает в Интернет через IP-адрес одного из наших серверов. Стриминговые сервисы и сайты видят только IP-адрес Private Internet Access, а не адрес вашего устройства."
      ],
      "Does Private Internet Access limit the bandwidth that streamers can use?": [
         null,
         "Ограничивает ли Private Internet Access пропускную способность канала для стриминга?"
      ],
      "Private Internet Access is the premier no log VPN - this means that we don’t even know if you are streaming. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Private Internet Access — это топовый VPN-сервис без логов; мы даже не знаем, что вы смотрите через стриминг. Все наши серверы обеспечивают неограниченную пропускную способность каналов, и мы не ограничиваем скорость для наших пользователей. Вот так. Регистрируйтесь в Private Internet Access сегодня, чтобы наслаждаться неограниченным Интернетом без цензуры."
      ],
      "Will Private Internet Access be blocked on my favorite streaming service or website?": [
         null,
         "Может ли мой любимый стриминговый сервис или сайт заблокировать Private Internet Access?"
      ],
      " Private Internet Access believes in an open internet and is always working to make sure that no blocks exist for our VPN users. Use our service today to see why Private Internet Access is the best VPN for streaming online.": [
         null,
         " Мы в Private Internet Access верим в свободный Интернет и постоянно работаем над тем, чтобы наши пользователи VPN не сталкивались ни с какими блокировками. Воспользуйтесь нашей службой сегодня, чтобы убедиться, что Private Internet Access является лучшим VPN для стриминга."
      ],
      "Save <b>%s%</b><br><span class=\"lines\">& get <b>%s extra months <span>free</span></b></span>": [
         null,
         "Сэкономьте <b>%s%</b><br><span class=\"lines\">и получите <b>%s доп. мес. <span>бесплатно</span></b></span>"
      ],
      "Save <b>%s%</b> & get<br><b class=\"lines\">%s extra months</b><br><span>free</span>": [
         null,
         "Сэкономьте <b>%s%</b> и получите<br><b class=\"lines\">%s доп. мес.</b><br><span>бесплатно</span>"
      ],
      "Save <b>%s%</b><br>& get<br><b class=\"lines\">%s extra months</b><br><span>free</span>": [
         null,
         "Сэкономьте <b>%s%</b><br>и получите<br><b class=\"lines\">%s доп. мес.</b><br><span>бесплатно</span>"
      ],
      "for only <b class=\"mx-md-3 mx-2\"><span>%s</span><span>%s</span></b> /month": [
         null,
         "всего за <b class=\"mx-md-3 mx-2\"><span>%s</span><span>%s</span></b> в мес."
      ],
      "The VPN recommended by<br><b>%s</b>": [
         null,
         "VPN, рекомендованный<br><b>%s</b>"
      ],
      "Save %s% Now": [
         null,
         "Экономьте %s% сейчас"
      ],
      "Sign-up now & get %s extra months <span class='text-uppercase'>free</span>": [
         null,
         "Купите подписку сейчас и получите %s доп. мес. <span class='text-uppercase'>бесплатно</span>"
      ],
      "Try Out the World's<br><b>Most <span class='color_pia_light_orange'>Trusted</span><br>& <span class='color_pia_light_orange'>Secure VPN</span></b>": [
         null,
         "Опробуйте<br><b>самый <span class='color_pia_light_orange'>надежный</span><br>и <span class='color_pia_light_orange'>безопасный VPN</span> в мире</b>"
      ],
      "Try it risk free! <i class='fas fa-award'></i> %s day money back guarantee": [
         null,
         "Попробуйте без риска! <i class='fas fa-award'></i> %s-дневная гарантия возврата денег"
      ],
      "Discover The Safest Way to Get<br><b><span class=\"text-uppercase color_pia_light_orange\">Total Online Privacy</span></b><br>Get Private Internet Access VPN": [
         null,
         "Оцените самый безопасный способ обеспечить<br><b><span class=\"text-uppercase color_pia_light_orange\">полную онлайн-приватность</span></b><br>Загрузите Private Internet Access VPN"
      ],
      "100%</span> No Logs & Records": [
         null,
         "Политика</span> полного отсутствия логов и записей"
      ],
      "Rated as the Best VPN Service In<br><b><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "Признан лучшим VPN-сервисом:<br><b><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "No Logs": [
         null,
         "Нет логов"
      ],
      "Military-Grade Encryption": [
         null,
         "Стойкое шифрование"
      ],
      "%s Devices": [
         null,
         "%s устройств"
      ],
      "Using <span>Zoom</span> for video calls?": [
         null,
         "Используете <span>Zoom</span> для видеозвонков?"
      ],
      "Make sure your online activity stays hidden": [
         null,
         "Убедитесь, что ваша онлайн-активность остается невидимой"
      ],
      "Why use a VPN with Zoom?": [
         null,
         "Зачем использовать VPN для Zoom?"
      ],
      "Zoom has been accused of sharing user data with Facebook, they lied about the app having end-to-end encryption, and the app allowed meeting hosts to track attendees.<br><br><b>So get Private Internet Access to encrypt your data, hide your IP address and become anonymous while using Zoom.</b>": [
         null,
         "Zoom упрекали в передаче пользовательских данных компании Facebook, они лгали о том, что приложение использует сквозное шифрование, а также приложение позволяло организаторам собраний отслеживать участников.<br><br><b>Поэтому используйте Private Internet Access, чтобы зашифровать свои данные, скрыть свой IP-адрес и сохранить анонимность при использовании Zoom.</b>"
      ],
      "First-rate VPN service:": [
         null,
         "VPN-сервис высшего класса:"
      ],
      "Bypass censorship and unblock streaming content and sites you don't have access to. Discover geo-restricted content on platforms like Netflix, HBO GO, Amazon Prime, Disney Plus, and more.": [
         null,
         "Обходите цензуру, разблокируйте стриминговый контент и заблокированные сайты. Получите доступ к контенту с региональными ограничениями на таких платформах, как Netflix, HBO GO, Amazon Prime, Disney Plus и так далее."
      ],
      "Lightning-fast Internet": [
         null,
         "Молниеносная скорость Интернета"
      ],
      "Private Internet Access provides the highest speeds possible. So, you can have calls without any buffering or connectivity issues.": [
         null,
         "Private Internet Access обеспечивает высочайшую возможную скорость, поэтому ваши звонки будут проходить без буферизации и без каких-либо проблем с подключением."
      ],
      "Unbreakable Encryption": [
         null,
         "Взломостойкое шифрование"
      ],
      "Encrypt your data to make sure nobody can hack your connection and get access to your data.": [
         null,
         "Используйте шифрование данных, чтобы никто не смог взломать ваше подключение и получить к ним доступ."
      ],
      "Become Anonymous": [
         null,
         "Сохраните анонимность"
      ],
      "Browse anonymously with a private IP address and protect yourself from your companies that want to get their hands on your data.": [
         null,
         "Посещайте сайты анонимно, используя приватный IP-адрес, и защитите себя от компаний, которые хотят заполучить ваши данные."
      ],
      "Connect in 4 easy steps:": [
         null,
         "Подключитесь за 4 простых шага:"
      ],
      "<b>Turn on</b><br>Private Internet Access and connect": [
         null,
         "<b>Включите</b><br>Private Internet Access и подключитесь"
      ],
      "You’re ready to start using <span>Private Internet Access</span> with zoom": [
         null,
         "Вы готовы к использованию <span>Private Internet Access</span> с Zoom"
      ],
      "Use PIA on up to %s devices simultaneously": [
         null,
         "Используйте PIA на %s устройствах одновременно"
      ],
      "%s-Day money back guarantee": [
         null,
         "%s-дневная гарантия возврата денег"
      ],
      "<span class=\"color_pia_green\">Frequently asked questions</span>": [
         null,
         "<span class=\"color_pia_green\">Часто задаваемые вопросы</span>"
      ],
      "How to use Private Internet Access with Zoom?": [
         null,
         "Как использовать Private Internet Access с Zoom?"
      ],
      "First download and install the Private Internet Access app on the device you're planning to use for your Zoom call. Then create and verify your account. Sing-in to the PIA app using your username and password and connect to a server by clicking or tapping the big, round power button. You're now completely anonymous and protected, all that's left to do is to fire-up your Zoom app and join your conference.": [
         null,
         "Сначала загрузите и установите приложение Private Internet Access на устройство, которые вы используете для звонков в Zoom. Затем создайте и подтвердите аккаунт. Выполните вход в приложении PIA, используя имя пользователя и пароль, затем подключитесь к серверу, нажав большую круглую кнопку включения. Теперь вы полностью анонимны и защищены. Остается запустить приложение Zoom и присоединиться к конференции."
      ],
      "Private Internet Access has a no questions asked %s-day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "У Private Internet Access действует гарантия возврата денег в течение %s дней без лишних вопросов. Приобретайте подписку Private Internet Access сегодня, чтобы сразу начать просмотр стриминга!"
      ],
      "Is it safe to use Private Internet Access?": [
         null,
         "Безопасно ли использовать Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. Private Internet access never records any of its users' data. We follow a strict no-logs policy that ensures we never keep any logs of our users' activities online.": [
         null,
         "Да. Private Internet Access предоставляет службу VPN с серверами в %s местоположениях, расположенных в %s странах, включая США, Великобританию, Японию и другие. Private Internet Access никогда не записывает никакие пользовательские данные. Мы следуем строгой политике отсутствия журналов, то есть никогда не храним никакие логи онлайн-активности наших пользователей."
      ],
      "Will my IP address be exposed while using Zoom?": [
         null,
         "Будет ли мой IP-адрес доступен посторонним при использовании Zoom?"
      ],
      "No. By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Streaming services, apps and websites will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Нет. При использовании Private Internet Access ваш трафик шифруется в VPN-тоннеле и попадает в Интернет через IP-адрес одного из наших серверов. Стриминговые сервисы, приложения и сайты видят только IP-адрес Private Internet Access, а не адрес вашего устройства."
      ],
      "Get Unbreakable Online Security With<br><b>The Best VPN for<br><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "Получите непробиваемую онлайн-защиту с<br><b>Лучшим VPN для<br><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "Unblock <span>new streaming content</span><br><small>on your <span>Fire Stick TV</span></small>": [
         null,
         "Разблокируйте <span>новый стриминговый контент</span><br><small>на вашем <span>Fire TV Stick</span></small>"
      ],
      "with Private Internet Access VPN": [
         null,
         "с Private Internet Access VPN"
      ],
      "Tired of watching the same shows and movies on streaming platforms?": [
         null,
         "Надоело смотреть одни и те же сериалы и фильмы на стриминговых платформах?"
      ],
      "Private Internet Access changes your IP address to one from a different location so you can discover new streaming properties.<br><b>Sign up to Private Internet Access today and unblock tons of new content from different countries around the world.</b>": [
         null,
         "Private Internet Access меняет ваш IP-адрес на адрес из другого местоположения, так что вы получаете доступ к новому стриминговому содержимому.<br><b>Зарегистрируйтесь в Private Internet Access сегодня и получите доступ к огромным медиатекам разных стран.</b>"
      ],
      "Stream new content on your Fire Stick TV:": [
         null,
         "Смотрите новый стриминговый контент на своем Fire TV Stick:"
      ],
      "A must have for your Fire Stick TV": [
         null,
         "Необходимая вещь для Fire TV Stick"
      ],
      "Access content from all over the world": [
         null,
         "Доступ к контенту по всему миру"
      ],
      "Private Internet Access has thousands of servers in over %s countries world-wide. Connect to either one of them and discover new films, TV shows and documentaries.": [
         null,
         "У Private Internet Access есть тысячи серверов в более чем %s различных странах. Подключайтесь к любому серверу и наслаждайтесь новыми фильмами, сериалами и документалистикой."
      ],
      "Override geographical restrictions": [
         null,
         "Обход региональных ограничений"
      ],
      "Connect to a server from a different country with just one click, straight from the Private Internet Access app, and unblock content that's not available in your country.": [
         null,
         "Подключайтесь к серверу в другой стране одним нажатием прямо из приложения Private Internet Access и разблокируйте контент, недоступный в вашей стране."
      ],
      "With just 1 Private Internet Access VPN subscription you can use the app on up to %s devices at the same time. This way you can unblock content on anything that has a screen and an internet connection.": [
         null,
         "Всего одна VPN-подписка Private Internet Access позволяет использовать приложение одновременно на %s устройствах. Так что вы сможете разблокировать контент на любом устройстве, у которого есть экран и доступ в Интернет."
      ],
      "All it takes are 4 easy steps:": [
         null,
         "Для этого достаточно 4 простых действий:"
      ],
      "Watch your favorite movies and shows in minutes": [
         null,
         "Всего несколько минут, и вы будете смотреть любимые фильмы и сериалы"
      ],
      "Got questions about streaming? <span class=\"color_pia_green\">Here are a few of our FAQs</span>": [
         null,
         "Есть вопросы о стриминге? <span class=\"color_pia_green\">Вот ответы на часто задаваемые вопросы</span>"
      ],
      "Private Internet Access is the premier no log VPN - this means that we don’t even know if you are streaming. All our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Private Internet Access — это топовый VPN-сервис без логов; мы даже не знаем, что вы смотрите через стриминг. Все наши серверы обеспечивают неограниченную пропускную способность каналов, и мы не ограничиваем скорость для наших пользователей. Вот так. Регистрируйтесь в Private Internet Access сегодня, чтобы наслаждаться неограниченным Интернетом без цензуры."
      ],
      "Private Internet Access believes in an open internet and is always working to make sure that no blocks exist for our VPN users. Use our service today to see why Private Internet Access is the best VPN for streaming online.": [
         null,
         "Мы в Private Internet Access верим в свободный Интернет и постоянно работаем над тем, чтобы наши пользователи VPN не сталкивались ни с какими блокировками. Воспользуйтесь нашей службой сегодня, чтобы убедиться, что Private Internet Access является лучшим VPN для стриминга."
      ],
      "Discover an endless amount of <br><b>streaming content</b> on <span>KODI</span>": [
         null,
         "Исследуйте бесконечный список <br><b>стримингового контента</b> в <span>KODI</span>"
      ],
      "Access geo-blocked TV shows and movies <br>while staying anonymous online": [
         null,
         "Получайте доступ к сериалам и фильмам, для которых установлены <br>региональные ограничения, оставаясь анонимным онлайн"
      ],
      "Don’t get into trouble while streaming on Kodi": [
         null,
         "Избегайте неприятностей, используя Kodi"
      ],
      "Whether you use Kodi TV to watch your personal content library or for the titles you find on their website, make sure nobody will be able to find out what you're doing online.<br><b>Why? </b>Streaming content on Kodi works is basically P2P torrenting which might be illegal in your country. Luckily, with the <b>Private Internet Access VPN</b>, you can make all your internet activity totally anonymous. So, you'll never have any trouble with governmental authorities while using Kodi.": [
         null,
         "Когда вы используете Kodi TV для просмотра собственного контента или фильмов, найденных на их сайте, стоит убедиться, что никто не может узнать, что вы делаете онлайн.<br><b>Почему? </b>Стриминговый контент в Kodi обычно является торрентами P2P, использование которых может быть запрещено в вашей стране. К счастью, при использовании <b>Private Internet Access VPN</b> вы можете сделать всю свою активность в Интернете анонимной. Поэтому вы никогда не попадете в неприятности с властями при использовании Kodi."
      ],
      "P2P traffic allowed": [
         null,
         "Трафик P2P разрешен"
      ],
      "Private Internet Access supports P2P protocols such as torrents.": [
         null,
         "Private Internet Access поддерживает P2P-протоколы, например торренты."
      ],
      "No Logs Policy": [
         null,
         "Политика отсутствия логов"
      ],
      "Private Internet Access never records its users' online activity and never keeps any logs, so you'll have complete anonymity from the moment you connect to our servers.": [
         null,
         "Private Internet Access никогда не записывает онлайн-активность пользователей и никогда не хранит никакие логи, поэтому с момента подключения к нашим серверам ваши действия становятся полностью анонимными."
      ],
      "Bypass geographical restrictions": [
         null,
         "Обход региональных ограничений"
      ],
      "Once you have chosen the location you want to use the internet from, Private Internet Access will always be one click away.": [
         null,
         "После того как вы выберете местоположение, из которого ваш трафик попадает в Интернет, для использования Private Internet Access достаточно одного нажатия."
      ],
      "Use Kodi TV on multiple devices": [
         null,
         "Используйте Kodi TV на нескольких устройствах"
      ],
      "One Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your whole family!": [
         null,
         "Одну подписку Private Internet Access можно одновременно использовать на нескольких устройствах (до %s). Используйте Private Internet Access совместно со всеми членами семьи!"
      ],
      "Watch Kodi anonymously in 4 easy steps": [
         null,
         "Смотрите Kodi анонимно, выполнив 4 простых действия"
      ],
      "You’re ready to start using <span>Private Internet Access</span> with your Kodi TV": [
         null,
         "Вы готовы к использованию <span>Private Internet Access</span> с Kodi TV"
      ],
      "Got questions? <span class=\"color_pia_green\">Check out our FAQs</span>": [
         null,
         "Есть вопросы? <span class=\"color_pia_green\">Посмотрите ответы на часто задаваемые вопросы</span>"
      ],
      "Is it safe to use Kodi with Private Internet Access?": [
         null,
         "Безопасно ли использовать Kodi с Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to use Kodi without Private Internet Access?": [
         null,
         "Да. Private Internet Access предоставляет службу VPN с серверами в %s местоположениях, расположенных в %s странах, включая США, Великобританию, Японию и другие. Более правильный вопрос: безопасно ли использовать Kodi без Private Internet Access?"
      ],
      "Yes. Private Internet Access has a no questions asked %s-day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Да. В Private Internet Access действует гарантия возврата денег в течение %s дней без лишних вопросов. Приобретайте подписку Private Internet Access сегодня, чтобы сразу начать просмотр стриминга!"
      ],
      "Will my IP address be exposed while using Kodi add-ons?": [
         null,
         "Будет ли мой IP-адрес доступен посторонним при использовании дополнений Kodi?"
      ],
      "No. By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Any service or website accessed will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Нет. При использовании Private Internet Access ваш трафик шифруется в VPN-тоннеле и попадает в Интернет через IP-адрес одного из наших серверов. Любой сервис или сайт видит только IP-адрес Private Internet Access, а не адрес вашего устройства."
      ],
      "Does Private Internet Access limit bandwidth for Kodi users?": [
         null,
         "Ограничивает ли Private Internet Access пропускную способность для пользователей Kodi?"
      ],
      "No. Private Internet Access is the premier no log VPN - this means that we don’t even know what software you use in conjunction with ours. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Нет. Private Internet Access — это топовый VPN-сервис без логов; мы даже не знаем, какое программное обеспечение вы используете в сочетании с нашим ПО. Все наши серверы обеспечивают неограниченную пропускную способность каналов, и мы не ограничиваем скорость для наших пользователей. Вот так. Регистрируйтесь в Private Internet Access сегодня, чтобы наслаждаться неограниченным Интернетом без цензуры."
      ],
      "Experience the Best Way<br><b>to Stay <span class='color_pia_light_orange'><span class='text-uppercase'>Safe</span> on Public Wi-Fi</span><br>& <span class='color_pia_light_orange text-uppercase'>Never Risk</span> Getting Hacked </b>": [
         null,
         "Оцените лучший способ<br><b>оставаться <span class='color_pia_light_orange'><span class='text-uppercase'>в безопасности</span> при использовании общедоступных сетей Wi-Fi</span><br>и <span class='color_pia_light_orange text-uppercase'>никогда не рисковать</span> возможностью быть взломанным </b>"
      ]
   }
}
;
