;var json_locale_data = {
   "messages": {
      "": {
         "Project-Id-Version": " pia-lps\nLanguage-Team: Danish\nReport-Msgid-Bugs-To: \nPO-Revision-Date: 2020-04-19 09:28\nLanguage: da_DK\nMIME-Version: 1.0\nContent-Type: text/plain; charset=UTF-8\nContent-Transfer-Encoding: 8bit\nPOT-Creation-Date: 2020-04-12 15:02+0000\nPlural-Forms: nplurals=2; plural=(n != 1);\nX-Crowdin-Project: pia-lps\nX-Crowdin-Language: da\nX-Crowdin-File: messages.pot\n"
      },
      "Afghanistan": [
         null,
         "Afghanistan"
      ],
      "Åland Islands": [
         null,
         "Ålandsøerne"
      ],
      "Albania": [
         null,
         "Albanien"
      ],
      "Algeria": [
         null,
         "Algeriet"
      ],
      "American Samoa": [
         null,
         "Amerikansk Samoa"
      ],
      "Andorra": [
         null,
         "Andorra"
      ],
      "Angola": [
         null,
         "Angola"
      ],
      "Anguilla": [
         null,
         "Anguilla"
      ],
      "Antarctica": [
         null,
         "Antarktis"
      ],
      "Antigua and Barbuda": [
         null,
         "Antigua og Barbuda"
      ],
      "Argentina": [
         null,
         "Argentina"
      ],
      "Armenia": [
         null,
         "Armenien"
      ],
      "Aruba": [
         null,
         "Aruba"
      ],
      "Australia": [
         null,
         "Australien"
      ],
      "Austria": [
         null,
         "Østrig"
      ],
      "Azerbaijan": [
         null,
         "Aserbajdsjan"
      ],
      "Bahamas": [
         null,
         "Bahamas"
      ],
      "Bahrain": [
         null,
         "Bahrain"
      ],
      "Bangladesh": [
         null,
         "Bangladesh"
      ],
      "Barbados": [
         null,
         "Barbados"
      ],
      "Belarus": [
         null,
         "Hviderusland"
      ],
      "Belgium": [
         null,
         "Belgien"
      ],
      "Belize": [
         null,
         "Belize"
      ],
      "Benin": [
         null,
         "Benin"
      ],
      "Bermuda": [
         null,
         "Bermuda"
      ],
      "Bhutan": [
         null,
         "Bhutan"
      ],
      "Bolivia": [
         null,
         "Bolivia"
      ],
      "Caribbean Netherlands": [
         null,
         "Caribisk Nederlandene"
      ],
      "Bosnia and Herzegovina": [
         null,
         "Bosnien-Hercegovina"
      ],
      "Botswana": [
         null,
         "Botswana"
      ],
      "Bouvet Island": [
         null,
         "Bouvetøen"
      ],
      "Brazil": [
         null,
         "Brasilien"
      ],
      "British Indian Ocean Territory": [
         null,
         "Britisk territorium i det indiske ocean"
      ],
      "Brunei Darussalam": [
         null,
         "Brunei Darussalam"
      ],
      "Bulgaria": [
         null,
         "Bulgarien"
      ],
      "Burkina Faso": [
         null,
         "Burkina Faso"
      ],
      "Burundi": [
         null,
         "Burundi"
      ],
      "Cabo Verde": [
         null,
         "Kap Verde"
      ],
      "Cambodia": [
         null,
         "Cambodja"
      ],
      "Cameroon": [
         null,
         "Cameroun"
      ],
      "Canada": [
         null,
         "Canada"
      ],
      "Cayman Islands": [
         null,
         "Caymanøerne"
      ],
      "Central African Republic": [
         null,
         "Den Centralafrikanske Republik"
      ],
      "Chad": [
         null,
         "Chad"
      ],
      "Chile": [
         null,
         "Chile"
      ],
      "China": [
         null,
         "Kina"
      ],
      "Christmas Island": [
         null,
         "Juleøen"
      ],
      "Cocos (Keeling) Islands": [
         null,
         "Cocosøerne (Keelingøerne)"
      ],
      "Colombia": [
         null,
         "Colombia"
      ],
      "Comoros": [
         null,
         "Comorerne"
      ],
      "Congo": [
         null,
         "Congo"
      ],
      "Democratic Republic of the Congo": [
         null,
         "Den Demokratiske Republik Congo"
      ],
      "Cook Islands": [
         null,
         "Cookøerne"
      ],
      "Costa Rica": [
         null,
         "Costa Rica"
      ],
      "Côte d'Ivoire": [
         null,
         "Elfenbenskysten"
      ],
      "Croatia": [
         null,
         "Kroatien"
      ],
      "Cuba": [
         null,
         "Cuba"
      ],
      "Curaçao": [
         null,
         "Curaçao"
      ],
      "Cyprus": [
         null,
         "Cypern"
      ],
      "Czechia": [
         null,
         "Tjekkiet"
      ],
      "Denmark": [
         null,
         "Danmark"
      ],
      "Djibouti": [
         null,
         "Djibouti"
      ],
      "Dominica": [
         null,
         "Dominica"
      ],
      "Dominican Republic": [
         null,
         "Dominikanske Republik"
      ],
      "Ecuador": [
         null,
         "Ecuador"
      ],
      "Egypt": [
         null,
         "Egypten"
      ],
      "El Salvador": [
         null,
         "El Salvador"
      ],
      "Equatorial Guinea": [
         null,
         "Ækvatorialguinea"
      ],
      "Eritrea": [
         null,
         "Eritrea"
      ],
      "Estonia": [
         null,
         "Estland"
      ],
      "Eswatini": [
         null,
         "Swaziland"
      ],
      "Ethiopia": [
         null,
         "Etiopien"
      ],
      "Falkland Islands (Malvinas)": [
         null,
         "Falklandsøerne"
      ],
      "Faroe Islands": [
         null,
         "Færøerne"
      ],
      "Fiji": [
         null,
         "Fiji"
      ],
      "Finland": [
         null,
         "Finland"
      ],
      "France": [
         null,
         "Frankrig"
      ],
      "French Guiana": [
         null,
         "Fransk Guyana"
      ],
      "French Polynesia": [
         null,
         "Fransk Polynesien"
      ],
      "French Southern Territories": [
         null,
         "Franske Sydlige og Antarktiske Territorier"
      ],
      "Gabon": [
         null,
         "Gabon"
      ],
      "Gambia": [
         null,
         "Gambia"
      ],
      "Georgia": [
         null,
         "Georgien"
      ],
      "Germany": [
         null,
         "Tyskland"
      ],
      "Ghana": [
         null,
         "Ghana"
      ],
      "Gibraltar": [
         null,
         "Gibraltar"
      ],
      "Greece": [
         null,
         "Grækenland"
      ],
      "Greenland": [
         null,
         "Grønland"
      ],
      "Grenada": [
         null,
         "Grenada"
      ],
      "Guadeloupe": [
         null,
         "Guadeloupe"
      ],
      "Guam": [
         null,
         "Guam"
      ],
      "Guatemala": [
         null,
         "Guatemala"
      ],
      "Guernsey": [
         null,
         "Guernsey"
      ],
      "Guinea": [
         null,
         "Guinea"
      ],
      "Guinea-Bissau": [
         null,
         "Guinea-Bissau"
      ],
      "Guyana": [
         null,
         "Guyana"
      ],
      "Haiti": [
         null,
         "Haiti"
      ],
      "Heard Island and McDonald Islands": [
         null,
         "Heard- og McDonaldøerne"
      ],
      "Holy See": [
         null,
         "Den Hellige Stol"
      ],
      "Honduras": [
         null,
         "Honduras"
      ],
      "Hong Kong": [
         null,
         "Hong Kong"
      ],
      "Hungary": [
         null,
         "Ungarn"
      ],
      "Iceland": [
         null,
         "Island"
      ],
      "India": [
         null,
         "Indien"
      ],
      "Indonesia": [
         null,
         "Indonesien"
      ],
      "Iran": [
         null,
         "Iran"
      ],
      "Iraq": [
         null,
         "Irak"
      ],
      "Ireland": [
         null,
         "Irland"
      ],
      "Isle of Man": [
         null,
         "Isle of Man"
      ],
      "Israel": [
         null,
         "Israel"
      ],
      "Italy": [
         null,
         "Italien"
      ],
      "Jamaica": [
         null,
         "Jamaica"
      ],
      "Japan": [
         null,
         "Japan"
      ],
      "Jersey": [
         null,
         "Jersey"
      ],
      "Jordan": [
         null,
         "Jordan"
      ],
      "Kazakhstan": [
         null,
         "Kasakhstan"
      ],
      "Kenya": [
         null,
         "Kenya"
      ],
      "Kiribati": [
         null,
         "Kiribati"
      ],
      "North Korea": [
         null,
         "Nordkorea"
      ],
      "South Korea": [
         null,
         "Sydkorea"
      ],
      "Kuwait": [
         null,
         "Kuwait"
      ],
      "Kyrgyzstan": [
         null,
         "Kirgisistan"
      ],
      "Laos": [
         null,
         "Laos"
      ],
      "Latvia": [
         null,
         "Letland"
      ],
      "Lebanon": [
         null,
         "Libanon"
      ],
      "Lesotho": [
         null,
         "Lesotho"
      ],
      "Liberia": [
         null,
         "Liberia"
      ],
      "Libya": [
         null,
         "Libyen"
      ],
      "Liechtenstein": [
         null,
         "Liechtenstein"
      ],
      "Lithuania": [
         null,
         "Litauen"
      ],
      "Luxembourg": [
         null,
         "Luxembourg"
      ],
      "Macao": [
         null,
         "Macao"
      ],
      "Madagascar": [
         null,
         "Madagaskar"
      ],
      "Malawi": [
         null,
         "Malawi"
      ],
      "Malaysia": [
         null,
         "Malaysia"
      ],
      "Maldives": [
         null,
         "Maldiverne"
      ],
      "Mali": [
         null,
         "Mali"
      ],
      "Malta": [
         null,
         "Malta"
      ],
      "Marshall Islands": [
         null,
         "Marshalløerne"
      ],
      "Martinique": [
         null,
         "Martinique"
      ],
      "Mauritania": [
         null,
         "Mauretanien"
      ],
      "Mauritius": [
         null,
         "Mauritius"
      ],
      "Mayotte": [
         null,
         "Mayotte"
      ],
      "Mexico": [
         null,
         "Mexico"
      ],
      "Federated States of Micronesia": [
         null,
         "Mikronesiens Forenede Stater"
      ],
      "Moldova": [
         null,
         "Moldova"
      ],
      "Monaco": [
         null,
         "Monaco"
      ],
      "Mongolia": [
         null,
         "Mongoliet"
      ],
      "Montenegro": [
         null,
         "Montenegro"
      ],
      "Montserrat": [
         null,
         "Montserrat"
      ],
      "Morocco": [
         null,
         "Marokko"
      ],
      "Mozambique": [
         null,
         "Mozambique"
      ],
      "Myanmar": [
         null,
         "Myanmar"
      ],
      "Namibia": [
         null,
         "Namibia"
      ],
      "Nauru": [
         null,
         "Nauru"
      ],
      "Nepal": [
         null,
         "Nepal"
      ],
      "Netherlands": [
         null,
         "Holland"
      ],
      "New Caledonia": [
         null,
         "Ny Kaledonien"
      ],
      "New Zealand": [
         null,
         "New Zealand"
      ],
      "Nicaragua": [
         null,
         "Nicaragua"
      ],
      "Niger": [
         null,
         "Niger"
      ],
      "Nigeria": [
         null,
         "Nigeria"
      ],
      "Niue": [
         null,
         "Niue"
      ],
      "Norfolk Island": [
         null,
         "Norfolk Island"
      ],
      "North Macedonia": [
         null,
         "Nordmakedonien"
      ],
      "Northern Mariana Islands": [
         null,
         "Nordmarianerne"
      ],
      "Norway": [
         null,
         "Norge"
      ],
      "Oman": [
         null,
         "Oman"
      ],
      "Pakistan": [
         null,
         "Pakistan"
      ],
      "Palau": [
         null,
         "Palau"
      ],
      "Panama": [
         null,
         "Panama"
      ],
      "Papua New Guinea": [
         null,
         "Papua Ny Guinea"
      ],
      "Paraguay": [
         null,
         "Paraguay"
      ],
      "Peru": [
         null,
         "Peru"
      ],
      "Philippines": [
         null,
         "Filippinerne"
      ],
      "Pitcairn": [
         null,
         "Pitcairn"
      ],
      "Poland": [
         null,
         "Polen"
      ],
      "Portugal": [
         null,
         "Portugal"
      ],
      "Puerto Rico": [
         null,
         "Puerto Rico"
      ],
      "Qatar": [
         null,
         "Qatar"
      ],
      "Réunion": [
         null,
         "Réunion"
      ],
      "Romania": [
         null,
         "Rumænien"
      ],
      "Russia": [
         null,
         "Rusland"
      ],
      "Rwanda": [
         null,
         "Rwanda"
      ],
      "Saint Barthélemy": [
         null,
         "Saint-Barthélemy"
      ],
      "Saint Helena, Ascension and Tristan da Cunha": [
         null,
         "Sankt Helena, Ascension og Tristan da Cunha"
      ],
      "Saint Kitts and Nevis": [
         null,
         "Saint Kitts og Nevis"
      ],
      "Saint Lucia": [
         null,
         "Saint Lucia"
      ],
      "Saint Martin (French part)": [
         null,
         "Saint Martin (fransk del)"
      ],
      "Saint Pierre and Miquelon": [
         null,
         "Saint Pierre og Miquelon"
      ],
      "Saint Vincent and the Grenadines": [
         null,
         "Saint Vincent og Grenadinerne"
      ],
      "Samoa": [
         null,
         "Samoa"
      ],
      "San Marino": [
         null,
         "San Marino"
      ],
      "Sao Tome and Principe": [
         null,
         "Sao Tome og Principe"
      ],
      "Saudi Arabia": [
         null,
         "Saudi-Arabien"
      ],
      "Senegal": [
         null,
         "Senegal"
      ],
      "Serbia": [
         null,
         "Serbien"
      ],
      "Seychelles": [
         null,
         "Seychellerne"
      ],
      "Sierra Leone": [
         null,
         "Sierra Leone"
      ],
      "Singapore": [
         null,
         "Singapore"
      ],
      "Sint Maarten (Dutch part)": [
         null,
         "Sint Maarten (hollandsk del)"
      ],
      "Slovakia": [
         null,
         "Slovakiet"
      ],
      "Slovenia": [
         null,
         "Slovenien"
      ],
      "Solomon Islands": [
         null,
         "Salomonøerne"
      ],
      "Somalia": [
         null,
         "Somalia"
      ],
      "South Africa": [
         null,
         "Sydafrika"
      ],
      "South Georgia and the South Sandwich Islands": [
         null,
         "Sydgeorgien og Sydsandwichøerne"
      ],
      "South Sudan": [
         null,
         "Sydsudan"
      ],
      "Spain": [
         null,
         "Spanien"
      ],
      "Sri Lanka": [
         null,
         "Sri Lanka"
      ],
      "Sudan": [
         null,
         "Sudan"
      ],
      "Suriname": [
         null,
         "Surinam"
      ],
      "Svalbard and Jan Mayen": [
         null,
         "Svalbard og Jan Mayen"
      ],
      "Sweden": [
         null,
         "Sverige"
      ],
      "Switzerland": [
         null,
         "Schweiz"
      ],
      "Syria": [
         null,
         "Syrien"
      ],
      "Taiwan, Province of China": [
         null,
         "Taiwan"
      ],
      "Tajikistan": [
         null,
         "Tadsjikistan"
      ],
      "Tanzania": [
         null,
         "Tanzania"
      ],
      "Thailand": [
         null,
         "Thailand"
      ],
      "Timor-Leste": [
         null,
         "Østtimor"
      ],
      "Togo": [
         null,
         "Togo"
      ],
      "Tokelau": [
         null,
         "Tokelau"
      ],
      "Tonga": [
         null,
         "Tonga"
      ],
      "Trinidad and Tobago": [
         null,
         "Trinidad og Tobago"
      ],
      "Tunisia": [
         null,
         "Tunesien"
      ],
      "Turkey": [
         null,
         "Tyrkiet"
      ],
      "Turkmenistan": [
         null,
         "Turkmenistan"
      ],
      "Turks and Caicos Islands": [
         null,
         "Turks- og Caicosøerne"
      ],
      "Tuvalu": [
         null,
         "Tuvalu"
      ],
      "Uganda": [
         null,
         "Uganda"
      ],
      "Ukraine": [
         null,
         "Ukraine"
      ],
      "the United Arab Emirates": [
         null,
         "Forenede Arabiske Emirater"
      ],
      "the United Kingdom": [
         null,
         "Det Forenede Kongerige"
      ],
      "the United States": [
         null,
         "Amerikas Forenede Stater"
      ],
      "the United States Minor Outlying Islands": [
         null,
         "USA’s ydre småøer"
      ],
      "Uruguay": [
         null,
         "Uruguay"
      ],
      "Uzbekistan": [
         null,
         "Usbekistan"
      ],
      "Vanuatu": [
         null,
         "Vanuatu"
      ],
      "Venezuela": [
         null,
         "Venezuela"
      ],
      "Viet Nam": [
         null,
         "Vietnam"
      ],
      "Virgin Islands (British)": [
         null,
         "Jomfruøerne (britiske)"
      ],
      "Virgin Islands (U.S.)": [
         null,
         "Jomfruøerne (amerikanske)"
      ],
      "Wallis and Futuna": [
         null,
         "Wallis og Futuna"
      ],
      "Western Sahara": [
         null,
         "Vestsahara"
      ],
      "Yemen": [
         null,
         "Yemen"
      ],
      "Zambia": [
         null,
         "Zambia"
      ],
      "Zimbabwe": [
         null,
         "Zimbabwe"
      ],
      "US Dollars": [
         null,
         "US dollars"
      ],
      "Join now": [
         null,
         "Tilmeld dig nu"
      ],
      "Based on %s reviews": [
         null,
         "Baseret på %s anmeldelser"
      ],
      "Read full review": [
         null,
         "Læs hele anmeldelsen"
      ],
      "Trusted By Companies Including:": [
         null,
         "Anvendt med tillid af virksomheder som f.eks.:"
      ],
      "World-class VPN service:": [
         null,
         "VPN-tjeneste i verdensklasse:"
      ],
      "Protect your online privacy and identity": [
         null,
         "Beskyt dit privatliv og din identitet online"
      ],
      "Browse anonymously with a private IP address and enable strong encryption to protect yourself from your ISP and public Wi-Fi networks.": [
         null,
         "Browse anonymt med en privat IP-adresse og aktiver stærk kryptering for at beskytte dig selv fra nysgerrige øjne hos din ISP og på offentlige Wi-Fi-netværk."
      ],
      "Unblock Streaming": [
         null,
         "Streaming uden blokering"
      ],
      "Get access to tons of new content on platforms like Netflix, Disney Plus, Amazon Prime Video, HBO GO, and more.": [
         null,
         "Få adgang til bunker af nyt indhold på platforme som Netflix, Disney Plus, Amazon Prime Video, HBO GO og mange flere."
      ],
      "The World's Leading VPN": [
         null,
         "Verdens førende VPN"
      ],
      "Private Internet Access provides the highest speeds and most encryption. So, get a perfect streaming experience without any buffering or slow loading times.": [
         null,
         "Private Internet Access tilbyder de højeste hastigheder og den bedste kryptering. Så du er sikret en perfekt streamingoplevelse uden buffering eller langsomme indlæsningstider."
      ],
      "Monthly": [
         null,
         "Månedligt"
      ],
      "Months": [
         null,
         "Måneder"
      ],
      "Yearly": [
         null,
         "Årligt"
      ],
      "Years": [
         null,
         "År"
      ],
      "Best value": [
         null,
         "Mest for pengene"
      ],
      "Save <b>%s%</b>": [
         null,
         "Spar <b>%s%</b>"
      ],
      "/mo": [
         null,
         "/må"
      ],
      "<del>%s</del> %s per month": [
         null,
         "<del>%s</del> %s pr. måned"
      ],
      "<del>%s</del> %s per %s months": [
         null,
         "<del>%s</del> %s pr. %s måneder"
      ],
      "<del>%s</del> %s per year": [
         null,
         "<del>%s</del> %s pr. år"
      ],
      "<del>%s</del>%s per %s years": [
         null,
         "<del>%s</del> %s pr. %s år"
      ],
      "Sign up now": [
         null,
         "Tilmeld dig nu"
      ],
      "Best<br>value": [
         null,
         "Mest for<br>pengene"
      ],
      "+%s extra<br>months free": [
         null,
         "+%s ekstra<br>måneder gratis"
      ],
      "All amounts are shown in %s, and any discounts reflect a reduction based on the current monthly service pricing at %s per month": [
         null,
         "Alle beløb vises i %s, og eventuelle rabatter afspejler en reduktion baseret på den aktuelle månedlige pris for tjenesten, som er %s pr. måned"
      ],
      "Why choose Private Internet Access?": [
         null,
         "Hvorfor vælge Private Internet Access?"
      ],
      "Total Privacy": [
         null,
         "Total fortrolighed"
      ],
      "Private Internet Access never keeps any records of users' online activities. You can rest assured knowing you have complete privacy, and nobody knows what you're doing online.": [
         null,
         "Private Internet Access registrerer aldrig brugernes onlineaktiviteter. Du kan roligt stole på, at din fortrolighed er sikret, og at ingen ved, hvad du laver online."
      ],
      "Easy to use": [
         null,
         "Nemt at bruge"
      ],
      "No matter your technical skills Private Internet Access is one of the easiest VPN apps out there. All it takes to connect is just 1 click or tap and your data will be encrypted instantly.": [
         null,
         "Uanset dine tekniske evner er Private Internet Access en af de mest letanvendelige VPN-apps, der findes. Du etablerer forbindelse med blot 1 enkelt klik, hvorefter alle dine data øjeblikkeligt bliver krypteret."
      ],
      "Secure VPN Protocols": [
         null,
         "Sikre VPN-protokoller"
      ],
      "Our app is powered by the latest and most advanced protocols like: PPTP, OpenVPN and L2TP/IPSec.": [
         null,
         "Vores app kører med de seneste og mest avancerede protokoller som: PPTP, OpenVPN og L2TP/IPSec."
      ],
      "%s devices simultaneously": [
         null,
         "%s enheder på samme tid"
      ],
      "Keep all your devices protected. With just 1 Private Internet Access VPN subscription you can connect on up to %s devices at the same time.": [
         null,
         "Hold alle dine enheder beskyttet. Med blot 1 Private Internet Access VPN-abonnement kan du gå på internettet med op til %s enheder på samme tid."
      ],
      "Our readers made it abundantly clear that Private Internet Access should be your first stop for protecting your private browsing data.": [
         null,
         "Vores læsere har klokkeklart givet udtryk for, at Private Internet Access bør være det første sted, du går hen, hvis du vil beskytte dine private browsingdata."
      ],
      "Private Internet Access | Anonymous VPN Service Provider": [
         null,
         "Private Internet Access | Udbyder af anonyme VPN-tjenester"
      ],
      "As recommended by": [
         null,
         "Anbefalet af"
      ],
      "Limited Time Only! <span class='color_pia_green'>Act Now</span>": [
         null,
         "Kun begrænset tid! <span class='color_pia_green'>Køb nu</span>"
      ],
      "%s<br><span>day</span>": [
         null,
         "%s<br><span>dages</span>"
      ],
      "money back guarantee": [
         null,
         "pengene tilbage-garanti"
      ],
      "Remember, you're not risking anything.": [
         null,
         "Husk, du løber ingen risiko her."
      ],
      "If you’re not 100% satisfied with <b>Private Internet Access</b>,<br>you have %s full days to ask for a refund and we’ll give you your money back.": [
         null,
         "Hvis du ikke er 100 % tilfreds med <b>Private Internet Access</b>,<br>har du %s fulde dage til at bede om at få beløbet refunderet, og så sender vi pengene tilbage."
      ],
      "FREE": [
         null,
         "GRATIS"
      ],
      "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 46.0271 120.4594)' font-size='17px'>EXTRA MONTHS</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>FREE": [
         null,
         "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 46.0271 120.4594)' font-size='17px'>EKSTRA MÅNEDER</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>GRATIS"
      ],
      "Connect to servers from all over the world": [
         null,
         "Etabler forbindelse til vores servere, uanset hvor i verden du befinder dig"
      ],
      "Servers": [
         null,
         "Servere"
      ],
      "Countries": [
         null,
         "Lande"
      ],
      "for ONLY": [
         null,
         "for KUN"
      ],
      "/month": [
         null,
         "/måned"
      ],
      "Proud Sponsor Of": [
         null,
         "Stolt sponsor af"
      ],
      "Compatible with all popular desktop and mobile operating systems": [
         null,
         "Kompatibel med alle populære operativsystemer til desktop-computere og mobile enheder"
      ],
      "For a Limited Time Only<br>Grab our": [
         null,
         "Kun i begrænset tid<br>Udnyt vores"
      ],
      "DISCOUNT": [
         null,
         "RABAT"
      ],
      "Experience": [
         null,
         "Oplev"
      ],
      "Total Online Freedom": [
         null,
         "Total frihed online"
      ],
      "Bypass censorship and unblock sites and content you don't have access to": [
         null,
         "Omgå censur og fjern blokeringen af hjemmesider og indhold, du ikke har adgang til"
      ],
      "Secure Protection on Public Wi-Fi": [
         null,
         "Sikker beskyttelse på offentlig Wi-Fi"
      ],
      "Keep your data safe and private while connecting to any poorly secured network": [
         null,
         "Hold dine data sikre og fortrolige, også når du tilslutter dig netværk med en lav grad af sikkerhed"
      ],
      "Unlimited Bandwidth & Fast Download Speeds": [
         null,
         "Ubegrænset båndbredde og hurtige downloadhastigheder"
      ],
      "Keep your downloads private and experience lightning fast speeds for P2P torrenting": [
         null,
         "Hold dine downloads private og oplev lynhurtige hastigheder ved P2P-torrenting"
      ],
      "Ad blocking": [
         null,
         "Annonceblokering"
      ],
      "With Private Internet Access you can block ads, trackers, and malware": [
         null,
         "Med Private Internet Access kan du blokere annoncer, trackere og malware"
      ],
      "Get Private Internet Access VPN": [
         null,
         "Få VPN fra Private Internet Access"
      ],
      "For a Limited Time Only": [
         null,
         "Kun i begrænset tid"
      ],
      "Your IP:": [
         null,
         "Din IP:"
      ],
      "Your Location:": [
         null,
         "Din placering:"
      ],
      "Your ISP:": [
         null,
         "Din ISP:"
      ],
      "Your Status:": [
         null,
         "Din status:"
      ],
      "You are not protected": [
         null,
         "Du er ikke beskyttet"
      ],
      "OOPS!": [
         null,
         "UPS!"
      ],
      "It looks like the page you were looking for isn't here anymore. Feel free to visit our homepage by clicking the button below.": [
         null,
         "Det ser ud til at den side, du søgte efter, ikke længere findes. Du kan gå til vores startside ved at trykke på knappen herunder."
      ],
      "Take Me To The Homepage": [
         null,
         "Gå til startsiden"
      ],
      "<small>Discover all the</small><br class=\"\"><b><span>streaming content</span></b><br>you’ve been missing out on": [
         null,
         "<small>Oplev alt det</small><br class=\"\"><b><span>streamingindhold</span></b><br>, du ikke havde adgang til før"
      ],
      "Unblock restricted content libraries easily<br> with Private Internet Access": [
         null,
         "Fjern nemt blokeringen af begrænsede indholdsbiblioteker<br> med Private Internet Access"
      ],
      "Try it risk free! <i class='fas fa-award'></i><span class='text-uppercase'> %s-day money back guarantee</span>": [
         null,
         "Prøv det helt uden risiko! <i class='fas fa-award'></i><span class='text-uppercase'> %s-dages pengene tilbage-garanti</span>"
      ],
      "Try it risk free!<br><span class='text-uppercase'>%s-day money back guarantee</span>": [
         null,
         "Prøv det helt uden risiko!<br><span class='text-uppercase'>%s-dages pengene tilbage-garanti</span>"
      ],
      "Private Internet Access VPN for %s% OFF": [
         null,
         "VPN fra Private Internet Access med %s% RABAT"
      ],
      "Discover Limitless Streaming Content": [
         null,
         "Få adgang til ubegrænset streaming af indhold"
      ],
      "Online streaming services and websites are becoming more popular every day all over the world. But sometimes geo-restrictions can limit the content you have access to. Don’t miss out on video streaming opportunities because of your location. <br><b class='mt-1 d-inline-block'>Sign up for Private Internet Access today!</b>": [
         null,
         "Online streamingtjenester og websites bliver mere og mere populære verden over. Men, nogen gange kan geo-blokeringer begrænse det indhold, du har adgang til. Gå ikke glip af videostreamingmuligheder på grund af din placering. <br><b class='mt-1 d-inline-block'>Tilmeld dig Private Internet Access allerede i dag!</b>"
      ],
      "Unblock content on:": [
         null,
         "Fjern blokeringen af indhold på:"
      ],
      "Sign up for Private Internet Access today!": [
         null,
         "Tilmeld dig Private Internet Access allerede i dag!"
      ],
      "A must have for your streaming needs": [
         null,
         "Et ’must have’ for at opfylde dine streaming-behov"
      ],
      "Unlimited bandwidth for streaming": [
         null,
         "Ubegrænset båndbredde til streaming"
      ],
      "Private Internet Access has no logs, never records online activity and does not throttle users.": [
         null,
         "Private Internet Access logger intet, registrerer aldrig din aktivitet online og begrænser aldrig brugerne."
      ],
      "One-click solution to override geographical restrictions": [
         null,
         "Annuller geografiske restriktioner med et enkelt klik"
      ],
      "Once you have chosen your virtual location, you can connect to it using Private Internet Access with just one click.": [
         null,
         "Så snart du har valgt din virtuelle placering, kan du etablere forbindelse til denne med Private Internet Access med kun et enkelt klik."
      ],
      "Stream on multiple devices": [
         null,
         "Streaming på flere enheder"
      ],
      "1 Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your entire family!": [
         null,
         "1 Private Internet Access-abonnementet kan bruges på op til %s enheder på samme tid. Del Private Internet Access med hele din familie!"
      ],
      "Four easy steps to stream with Private Internet Access": [
         null,
         "Fire nemme trin, og så er du i gang med at streame med Private Internet Access"
      ],
      "Start streaming with Private Internet Access in minutes": [
         null,
         "Start streaming med Private Internet Access på få minutter"
      ],
      "<b>Subscribe</b><br>to Private Internet Access": [
         null,
         "<b>Abonner</b><br>på Private Internet Access"
      ],
      "<b>Download</b><br>Private Internet Access": [
         null,
         "<b>Download</b><br>Private Internet Access"
      ],
      "<b>Login</b><br>on the Private Internet Access client with your username and password": [
         null,
         "<b>Login</b><br>på Private Internet Access klienten med dit brugernavn og din adgangskode"
      ],
      "<b>Turn on</b><br>Private Internet Access and connect to the location of your choice": [
         null,
         "<b>Aktivér</b><br>Private Internet Access, og etabler forbindelse til en placering helt efter dit eget ønske"
      ],
      "You’re ready to start streaming with <span>Private Internet Access</span>": [
         null,
         "Så er du klar til at starte streaming med <span>Private Internet Access</span>"
      ],
      "Private Internet Access Streaming FAQ": [
         null,
         "Ofte stillede spørgsmål om streaming med Private Internet Access"
      ],
      "Is it safe to stream with Private Internet Access?": [
         null,
         "Er det sikkert at streame med Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in 49 locations across 29 countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to stream without Private Internet Access?": [
         null,
         "Ja. Private Internet Access tilbyder en VPN-tjeneste med servere placeret 49 forskellige steder i 29 lande, herunder USA, Storbritannien, Japan og mange flere. Det rigtige spørgsmål er: Er det sikkert at streame uden Private Internet Access?"
      ],
      "Do you have a refund policy?": [
         null,
         "Har I en refusionspolitik?"
      ],
      "Private Internet Access has a no questions asked 7 day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Private Internet Access har en 7-dages pengene tilbage-garanti, hvor vi ikke stiller nogen som helst spørgsmål. Abonner på Private Internet Access allerede i dag, så kan du streame med det samme!"
      ],
      "Will my IP address be exposed to streaming services or websites?": [
         null,
         "Vil min IP-adresse kunne ses af streamingtjenester eller websites?"
      ],
      "By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Streaming services and websites will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Når du bruger Private Internet Access, bliver din trafik krypteret gennem vores VPN-tunnel, som har adgang til internettet fra en af vores serveres IP-adresser. Streamingtjenester og websites kan kun se vores Private Internet Access IP-adresse, ikke din enheds IP-adresse."
      ],
      "Does Private Internet Access limit the bandwidth that streamers can use?": [
         null,
         "Begrænser Private Internet Access den båndbredde, som streamere kan bruge?"
      ],
      "Private Internet Access is the premier no log VPN - this means that we don’t even know if you are streaming. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Private Internet Access er den førende VPN uden logning - det betyder, at vi ikke engang ved, at du streamer. Alle vores servere tilbyder ubegrænset båndbredde, og vi begrænser ikke vores brugere. Punktum. Tilmeld dig Private Internet Access allerede i dag for at udnytte det ubegrænsede internet uden censur."
      ],
      "Will Private Internet Access be blocked on my favorite streaming service or website?": [
         null,
         "Vil Private Internet Access blive blokeret på min favoritstreamingtjeneste eller website?"
      ],
      " Private Internet Access believes in an open internet and is always working to make sure that no blocks exist for our VPN users. Use our service today to see why Private Internet Access is the best VPN for streaming online.": [
         null,
         " Private Internet Access tror på et åbent internet og arbejder løbende på at sikre, at ingen af vores VPN-brugere bliver blokeret. Brug vores tjeneste i dag, og du vil se, hvorfor Private Internet Access er den bedste VPN til online streaming."
      ],
      "The VPN recommended by<br><b>%s</b>": [
         null,
         "VPN anbefalet af<br><b>%s</b>"
      ],
      "Save %s% Now": [
         null,
         "Spar %s% nu"
      ],
      "Sign-up now & get %s extra months <span class='text-uppercase'>free</span>": [
         null,
         "Tilmeld dig nu og få %s måneder ekstra <span class='text-uppercase'>gratis</span>"
      ],
      "Try Out the World's<br><b>Most <span class='color_pia_light_orange'>Trusted</span><br>& <span class='color_pia_light_orange'>Secure VPN</span></b>": [
         null,
         "Prøv verdens<br><b>mest <span class='color_pia_light_orange'>pålidelige</span><br>og <span class='color_pia_light_orange'>sikre VPN</span></b>"
      ],
      "Try it risk free! <i class='fas fa-award'></i> %s day money back guarantee": [
         null,
         "Prøv det helt uden risiko! <i class='fas fa-award'></i> %s-dages pengene tilbage-garanti"
      ],
      "Discover The Safest Way to Get<br><b><span class=\"text-uppercase color_pia_light_orange\">Total Online Privacy</span></b><br>Get Private Internet Access VPN": [
         null,
         "Opdag den sikreste metode til at opnå<br><b><span class=\"text-uppercase color_pia_light_orange\">total fortrolighed online</span></b><br>Hent Private Internet Access VPN"
      ],
      "100%</span> No Logs & Records": [
         null,
         "100 %</span> uden logs og registreringer"
      ],
      "Rated as the Best VPN Service In<br><b><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "Bedømt som den bedste VPN-tjeneste i<br><b><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "No Logs": [
         null,
         "Ingen logs"
      ],
      "Military-Grade Encryption": [
         null,
         "Military-grade kryptering"
      ],
      "%s Devices": [
         null,
         "%s enheder"
      ],
      "Get Unbreakable Online Security With<br><b>The Best VPN for<br><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "Få ubrydelig online sikkerhed med<br><b>den bedste VPN til<br><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "Discover an endless amount of <br><b>streaming content</b> on <span>KODI</span>": [
         null,
         "Opdag en endeløs mængde af <br><b>streamingindhold</b> på <span>KODI</span>"
      ],
      "Access geo-blocked TV shows and movies <br>while staying anonymous online": [
         null,
         "Få adgang til geo-blokerede TV-shows og film <br>mens du forbliver anonym online"
      ],
      "Don’t get into trouble while streaming on Kodi": [
         null,
         "Undgå problemer, når du streamer på Kodi"
      ],
      "Whether you use Kodi TV to watch your personal content library or for the titles you find on their website, make sure nobody will be able to find out what you're doing online.<br><b>Why? </b>Streaming content on Kodi works is basically P2P torrenting which might be illegal in your country. Luckily, with the <b>Private Internet Access VPN</b>, you can make all your internet activity totally anonymous. So, you'll never have any trouble with governmental authorities while using Kodi.": [
         null,
         "Sørg for, at ingen kan finde ud af, hvad du laver online, uanset om du bruger Kodi TV til at se indhold fra dit personlige bibliotek eller til at søge efter titler, som du kan finde på deres website.<br><b>Hvorfor? </b>Streaming af indhold på Kodi fungerer grundlæggende som P2P-torrenting, hvilket kan være ulovligt i dit land. Heldigvis kan du med <b>Private Internet Access VPN</b> gøre al din internetaktivitet fuldstændig anonym. Så du vil aldrig få problemer med myndighederne, når du bruger Kodi."
      ],
      "P2P traffic allowed": [
         null,
         "P2P-trafik tilladt"
      ],
      "Private Internet Access supports P2P protocols such as torrents.": [
         null,
         "Private Internet Access understøtter P2P-protokoller som f.eks. torrents."
      ],
      "No Logs Policy": [
         null,
         "Politik om ikke at logge noget som helst"
      ],
      "Private Internet Access never records its users' online activity and never keeps any logs, so you'll have complete anonymity from the moment you connect to our servers.": [
         null,
         "Private Internet Access registrerer aldrig brugernes onlineaktiviteter, og vi logfører intet, så du er sikret fuldstændig anonymitet fra det øjeblik, du etablerer forbindelse til vores servere."
      ],
      "Bypass geographical restrictions": [
         null,
         "Omgå geografiske restriktioner"
      ],
      "Once you have chosen the location you want to use the internet from, Private Internet Access will always be one click away.": [
         null,
         "Så snart du har valgt den placering, fra hvilken du vil tilgå internettet, er Private Internet Access altid tilgængelig med et enkelt klik."
      ],
      "Use Kodi TV on multiple devices": [
         null,
         "Brug Kodi TV på flere enheder"
      ],
      "1 Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your whole family!": [
         null,
         "1 Private Internet Access-abonnementet kan bruges på op til %s enheder på samme tid. Del Private Internet Access med hele din familie!"
      ],
      "Watch Kodi anonymously in 4 easy steps": [
         null,
         "4 nemme trin, og du kan se Kodi helt anonymt"
      ],
      "You’re ready to start using <span>Private Internet Access</span> with your Kodi TV": [
         null,
         "Du kan nu begynde at bruge <span>Private Internet Access</span> med Kodi TV"
      ],
      "Got questions? <span class=\"color_pia_green\">Check out our FAQs</span>": [
         null,
         "Spørgsmål? <span class=\"color_pia_green\">Tjek vores hyppigt stillede spørgsmål</span>"
      ],
      "Is it safe to use Kodi with Private Internet Access?": [
         null,
         "Er det sikkert at bruge Kodi med Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to use Kodi without Private Internet Access?": [
         null,
         "Ja. Private Internet Access tilbyder en VPN-tjeneste med servere placeret %s forskellige steder i %s lande, herunder USA, Storbritannien, Japan og mange flere. Det rigtige spørgsmål er: Er det sikkert at bruge Kodi uden Private Internet Access?"
      ],
      "Yes. Private Internet Access has a no questions asked %s-day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Ja. Private Internet Access har en %s-dages pengene tilbage-garanti, hvor vi ikke stiller nogen som helst spørgsmål. Abonner på Private Internet Access allerede i dag, så kan du streame med det samme!"
      ],
      "Will my IP address be exposed while using Kodi add-ons?": [
         null,
         "Vil min IP-adresse kunne ses, når jeg bruger Kodi add-ons?"
      ],
      "No. By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Any service or website accessed will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Nej. Når du bruger Private Internet Access, bliver din trafik krypteret gennem vores VPN-tunnel, som har adgang til internettet fra en af vores serveres IP-adresser. Uanset hvilken tjeneste eller website, du bruger, er det kun se vores Private Internet Access IP-adresse, der kan ses, ikke din enheds IP-adresse."
      ],
      "Does Private Internet Access limit bandwidth for Kodi users?": [
         null,
         "Begrænser Private Internet Access båndbredden for Kodi brugere?"
      ],
      "No. Private Internet Access is the premier no log VPN - this means that we don’t even know what software you use in conjunction with ours. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Nej. Private Internet Access er den førende VPN uden logning - det betyder, at vi ikke engang ved, hvilken software du bruger sammen med vores. Alle vores servere tilbyder ubegrænset båndbredde, og vi begrænser ikke vores brugere. Punktum. Tilmeld dig Private Internet Access allerede i dag for at udnytte det ubegrænsede internet uden censur."
      ],
      "Experience the Best Way<br><b>to Stay <span class='color_pia_light_orange'><span class='text-uppercase'>Safe</span> on Public Wi-Fi</span><br>& <span class='color_pia_light_orange text-uppercase'>Never Risk</span> Getting Hacked </b>": [
         null,
         "Oplev den bedste metode<br><b>til at opretholde <span class='color_pia_light_orange'><span class='text-uppercase'>sikkerheden</span> på offentlige Wi-Fi-netværk</span><br>og <span class='color_pia_light_orange text-uppercase'>helt eliminere risikoen</span> for at blive hacket </b>"
      ]
   }
}
;
