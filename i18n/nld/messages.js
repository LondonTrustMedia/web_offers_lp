;var json_locale_data = {
   "messages": {
      "": {
         "Project-Id-Version": " pia-lps\nLanguage-Team: Dutch\nReport-Msgid-Bugs-To: \nPO-Revision-Date: 2020-05-13 11:06\nLanguage: nl_NL\nMIME-Version: 1.0\nContent-Type: text/plain; charset=UTF-8\nContent-Transfer-Encoding: 8bit\nPOT-Creation-Date: 2020-05-13 10:52+0000\nPlural-Forms: nplurals=2; plural=(n != 1);\nX-Crowdin-Project: pia-lps\nX-Crowdin-Language: nl\nX-Crowdin-File: messages.pot\n"
      },
      "Afghanistan": [
         null,
         "Afghanistan"
      ],
      "Åland Islands": [
         null,
         "Ålandseilanden"
      ],
      "Albania": [
         null,
         "Albanië"
      ],
      "Algeria": [
         null,
         "Algerije"
      ],
      "American Samoa": [
         null,
         "Amerikaans Samoa"
      ],
      "Andorra": [
         null,
         "Andorra"
      ],
      "Angola": [
         null,
         "Angola"
      ],
      "Anguilla": [
         null,
         "Anguilla"
      ],
      "Antarctica": [
         null,
         "Antarctica"
      ],
      "Antigua and Barbuda": [
         null,
         "Antigua en Barbuda"
      ],
      "Argentina": [
         null,
         "Argentinië"
      ],
      "Armenia": [
         null,
         "Armenië"
      ],
      "Aruba": [
         null,
         "Aruba"
      ],
      "Australia": [
         null,
         "Australië"
      ],
      "Austria": [
         null,
         "Oostenrijk"
      ],
      "Azerbaijan": [
         null,
         "Azerbeidzjan"
      ],
      "Bahamas": [
         null,
         "Bahama’s"
      ],
      "Bahrain": [
         null,
         "Bahrein"
      ],
      "Bangladesh": [
         null,
         "Bangladesh"
      ],
      "Barbados": [
         null,
         "Barbados"
      ],
      "Belarus": [
         null,
         "Wit-Rusland"
      ],
      "Belgium": [
         null,
         "België"
      ],
      "Belize": [
         null,
         "Belize"
      ],
      "Benin": [
         null,
         "Benin"
      ],
      "Bermuda": [
         null,
         "Bermuda"
      ],
      "Bhutan": [
         null,
         "Bhutan"
      ],
      "Bolivia": [
         null,
         "Bolivia"
      ],
      "Caribbean Netherlands": [
         null,
         "Caribisch Nederland"
      ],
      "Bosnia and Herzegovina": [
         null,
         "Bosnië-Herzegovina"
      ],
      "Botswana": [
         null,
         "Botswana"
      ],
      "Bouvet Island": [
         null,
         "Bouveteiland"
      ],
      "Brazil": [
         null,
         "Brazilië"
      ],
      "British Indian Ocean Territory": [
         null,
         "Brits-Indisch oceaangebied"
      ],
      "Brunei Darussalam": [
         null,
         "Brunei Darussalam"
      ],
      "Bulgaria": [
         null,
         "Bulgarije"
      ],
      "Burkina Faso": [
         null,
         "Burkina Faso"
      ],
      "Burundi": [
         null,
         "Burundi"
      ],
      "Cabo Verde": [
         null,
         "Kaapverdië"
      ],
      "Cambodia": [
         null,
         "Cambodja"
      ],
      "Cameroon": [
         null,
         "Kameroen"
      ],
      "Canada": [
         null,
         "Canada"
      ],
      "Cayman Islands": [
         null,
         "Kaaimaneilanden"
      ],
      "Central African Republic": [
         null,
         "Centraal-Afrikaanse Republiek"
      ],
      "Chad": [
         null,
         "Tsjaad"
      ],
      "Chile": [
         null,
         "Chili"
      ],
      "China": [
         null,
         "China"
      ],
      "Christmas Island": [
         null,
         "Kersteiland"
      ],
      "Cocos (Keeling) Islands": [
         null,
         "Cocoseilanden (Keeling Islands)"
      ],
      "Colombia": [
         null,
         "Colombia"
      ],
      "Comoros": [
         null,
         "Comoren"
      ],
      "Congo": [
         null,
         "Congo"
      ],
      "Democratic Republic of the Congo": [
         null,
         "Democratische Republiek Congo"
      ],
      "Cook Islands": [
         null,
         "Cookeilanden"
      ],
      "Costa Rica": [
         null,
         "Costa Rica"
      ],
      "Côte d'Ivoire": [
         null,
         "Ivoorkust"
      ],
      "Croatia": [
         null,
         "Kroatië"
      ],
      "Cuba": [
         null,
         "Cuba"
      ],
      "Curaçao": [
         null,
         "Curaçao"
      ],
      "Cyprus": [
         null,
         "Cyprus"
      ],
      "Czechia": [
         null,
         "Tsjechië"
      ],
      "Denmark": [
         null,
         "Denemarken"
      ],
      "Djibouti": [
         null,
         "Djibouti"
      ],
      "Dominica": [
         null,
         "Dominica"
      ],
      "Dominican Republic": [
         null,
         "Dominicaanse Republiek"
      ],
      "Ecuador": [
         null,
         "Ecuador"
      ],
      "Egypt": [
         null,
         "Egypte"
      ],
      "El Salvador": [
         null,
         "El Salvador"
      ],
      "Equatorial Guinea": [
         null,
         "Equatoriaal-Guinea"
      ],
      "Eritrea": [
         null,
         "Eritrea"
      ],
      "Estonia": [
         null,
         "Estland"
      ],
      "Eswatini": [
         null,
         "Eswatini"
      ],
      "Ethiopia": [
         null,
         "Ethiopië"
      ],
      "Falkland Islands (Malvinas)": [
         null,
         "Falklandeilanden (Malvinas)"
      ],
      "Faroe Islands": [
         null,
         "Faeröer"
      ],
      "Fiji": [
         null,
         "Fiji"
      ],
      "Finland": [
         null,
         "Finland"
      ],
      "France": [
         null,
         "Frankrijk"
      ],
      "French Guiana": [
         null,
         "Frans-Guyana"
      ],
      "French Polynesia": [
         null,
         "Frans-Polynesië"
      ],
      "French Southern Territories": [
         null,
         "Franse zuidelijke gebieden"
      ],
      "Gabon": [
         null,
         "Gabon"
      ],
      "Gambia": [
         null,
         "Gambia"
      ],
      "Georgia": [
         null,
         "Georgië"
      ],
      "Germany": [
         null,
         "Duitsland"
      ],
      "Ghana": [
         null,
         "Ghana"
      ],
      "Gibraltar": [
         null,
         "Gibraltar"
      ],
      "Greece": [
         null,
         "Griekenland"
      ],
      "Greenland": [
         null,
         "Groenland"
      ],
      "Grenada": [
         null,
         "Grenada"
      ],
      "Guadeloupe": [
         null,
         "Guadeloupe"
      ],
      "Guam": [
         null,
         "Guam"
      ],
      "Guatemala": [
         null,
         "Guatemala"
      ],
      "Guernsey": [
         null,
         "Guernsey"
      ],
      "Guinea": [
         null,
         "Guinea"
      ],
      "Guinea-Bissau": [
         null,
         "Guinea-Bissau"
      ],
      "Guyana": [
         null,
         "Guyana"
      ],
      "Haiti": [
         null,
         "Haïti"
      ],
      "Heard Island and McDonald Islands": [
         null,
         "Heard en McDonaldeilanden"
      ],
      "Holy See": [
         null,
         "Heilige Stoel"
      ],
      "Honduras": [
         null,
         "Honduras"
      ],
      "Hong Kong": [
         null,
         "Hong Kong"
      ],
      "Hungary": [
         null,
         "Hongarije"
      ],
      "Iceland": [
         null,
         "IJsland"
      ],
      "India": [
         null,
         "India"
      ],
      "Indonesia": [
         null,
         "Indonesië"
      ],
      "Iran": [
         null,
         "Iran"
      ],
      "Iraq": [
         null,
         "Irak"
      ],
      "Ireland": [
         null,
         "Ierland"
      ],
      "Isle of Man": [
         null,
         "Eiland Man"
      ],
      "Israel": [
         null,
         "Israël"
      ],
      "Italy": [
         null,
         "Italië"
      ],
      "Jamaica": [
         null,
         "Jamaica"
      ],
      "Japan": [
         null,
         "Japan"
      ],
      "Jersey": [
         null,
         "Jersey"
      ],
      "Jordan": [
         null,
         "Jordanië"
      ],
      "Kazakhstan": [
         null,
         "Kazachstan"
      ],
      "Kenya": [
         null,
         "Kenia"
      ],
      "Kiribati": [
         null,
         "Kiribati"
      ],
      "North Korea": [
         null,
         "Noord-Korea"
      ],
      "South Korea": [
         null,
         "Zuid-Korea"
      ],
      "Kuwait": [
         null,
         "Koeweit"
      ],
      "Kyrgyzstan": [
         null,
         "Kirgizië"
      ],
      "Laos": [
         null,
         "Laos"
      ],
      "Latvia": [
         null,
         "Letland"
      ],
      "Lebanon": [
         null,
         "Libanon"
      ],
      "Lesotho": [
         null,
         "Lesotho"
      ],
      "Liberia": [
         null,
         "Liberia"
      ],
      "Libya": [
         null,
         "Libië"
      ],
      "Liechtenstein": [
         null,
         "Liechtenstein"
      ],
      "Lithuania": [
         null,
         "Litouwen"
      ],
      "Luxembourg": [
         null,
         "Luxemburg"
      ],
      "Macao": [
         null,
         "Macau"
      ],
      "Madagascar": [
         null,
         "Madagaskar"
      ],
      "Malawi": [
         null,
         "Malawi"
      ],
      "Malaysia": [
         null,
         "Maleisië"
      ],
      "Maldives": [
         null,
         "Malediven"
      ],
      "Mali": [
         null,
         "Mali"
      ],
      "Malta": [
         null,
         "Malta"
      ],
      "Marshall Islands": [
         null,
         "Marshalleilanden"
      ],
      "Martinique": [
         null,
         "Martinique"
      ],
      "Mauritania": [
         null,
         "Mauritanië"
      ],
      "Mauritius": [
         null,
         "Mauritius"
      ],
      "Mayotte": [
         null,
         "Mayotte"
      ],
      "Mexico": [
         null,
         "Mexico"
      ],
      "Federated States of Micronesia": [
         null,
         "Federale Staten van Micronesia"
      ],
      "Moldova": [
         null,
         "Moldavië"
      ],
      "Monaco": [
         null,
         "Monaco"
      ],
      "Mongolia": [
         null,
         "Mongolië"
      ],
      "Montenegro": [
         null,
         "Montenegro"
      ],
      "Montserrat": [
         null,
         "Montserrat"
      ],
      "Morocco": [
         null,
         "Marokko"
      ],
      "Mozambique": [
         null,
         "Mozambique"
      ],
      "Myanmar": [
         null,
         "Myanmar"
      ],
      "Namibia": [
         null,
         "Namibië"
      ],
      "Nauru": [
         null,
         "Nauru"
      ],
      "Nepal": [
         null,
         "Nepal"
      ],
      "Netherlands": [
         null,
         "Nederland"
      ],
      "New Caledonia": [
         null,
         "Nieuw-Caledonië"
      ],
      "New Zealand": [
         null,
         "Nieuw Zeeland"
      ],
      "Nicaragua": [
         null,
         "Nicaragua"
      ],
      "Niger": [
         null,
         "Niger"
      ],
      "Nigeria": [
         null,
         "Nigeria"
      ],
      "Niue": [
         null,
         "Niue"
      ],
      "Norfolk Island": [
         null,
         "Norfolkeiland"
      ],
      "North Macedonia": [
         null,
         "Noord-Macedonië"
      ],
      "Northern Mariana Islands": [
         null,
         "Noordelijke Marianen"
      ],
      "Norway": [
         null,
         "Noorwegen"
      ],
      "Oman": [
         null,
         "Oman"
      ],
      "Pakistan": [
         null,
         "Pakistan"
      ],
      "Palau": [
         null,
         "Palau"
      ],
      "Panama": [
         null,
         "Panama"
      ],
      "Papua New Guinea": [
         null,
         "Papoea-Nieuw-Guinea"
      ],
      "Paraguay": [
         null,
         "Paraguay"
      ],
      "Peru": [
         null,
         "Peru"
      ],
      "Philippines": [
         null,
         "Filipijnen"
      ],
      "Pitcairn": [
         null,
         "Pitcairn"
      ],
      "Poland": [
         null,
         "Polen"
      ],
      "Portugal": [
         null,
         "Portugal"
      ],
      "Puerto Rico": [
         null,
         "Puerto Rico"
      ],
      "Qatar": [
         null,
         "Qatar"
      ],
      "Réunion": [
         null,
         "Réunion"
      ],
      "Romania": [
         null,
         "Roemenië"
      ],
      "Russia": [
         null,
         "Rusland"
      ],
      "Rwanda": [
         null,
         "Rwanda"
      ],
      "Saint Barthélemy": [
         null,
         "Saint Barthélemy"
      ],
      "Saint Helena, Ascension and Tristan da Cunha": [
         null,
         "Sint-Helena, Ascension en Tristan da Cunha"
      ],
      "Saint Kitts and Nevis": [
         null,
         "Saint Kitts en Nevis"
      ],
      "Saint Lucia": [
         null,
         "Saint Lucia"
      ],
      "Saint Martin (French part)": [
         null,
         "Saint Martin (Franse deel)"
      ],
      "Saint Pierre and Miquelon": [
         null,
         "Saint Pierre en Miquelon"
      ],
      "Saint Vincent and the Grenadines": [
         null,
         "Saint Vincent en de Grenadines"
      ],
      "Samoa": [
         null,
         "Samoa"
      ],
      "San Marino": [
         null,
         "San Marino"
      ],
      "Sao Tome and Principe": [
         null,
         "Sao Tomé en Principe"
      ],
      "Saudi Arabia": [
         null,
         "Saoedi-Arabië"
      ],
      "Senegal": [
         null,
         "Senegal"
      ],
      "Serbia": [
         null,
         "Servië"
      ],
      "Seychelles": [
         null,
         "Seychellen"
      ],
      "Sierra Leone": [
         null,
         "Sierra Leone"
      ],
      "Singapore": [
         null,
         "Singapore"
      ],
      "Sint Maarten (Dutch part)": [
         null,
         "Sint Maarten (Nederlandse deel)"
      ],
      "Slovakia": [
         null,
         "Slowakije"
      ],
      "Slovenia": [
         null,
         "Slovenië"
      ],
      "Solomon Islands": [
         null,
         "Salomonseilanden"
      ],
      "Somalia": [
         null,
         "Somalië"
      ],
      "South Africa": [
         null,
         "Zuid-Afrika"
      ],
      "South Georgia and the South Sandwich Islands": [
         null,
         "Zuid-Georgia en de Zuidelijke Sandwicheilanden"
      ],
      "South Sudan": [
         null,
         "Zuid-Soedan"
      ],
      "Spain": [
         null,
         "Spanje"
      ],
      "Sri Lanka": [
         null,
         "Sri Lanka"
      ],
      "Sudan": [
         null,
         "Soedan"
      ],
      "Suriname": [
         null,
         "Suriname"
      ],
      "Svalbard and Jan Mayen": [
         null,
         "Spitsbergen en Jan Mayen"
      ],
      "Sweden": [
         null,
         "Zweden"
      ],
      "Switzerland": [
         null,
         "Zwitserland"
      ],
      "Syria": [
         null,
         "Syrië"
      ],
      "Taiwan, Province of China": [
         null,
         "Taiwan, provincie China"
      ],
      "Tajikistan": [
         null,
         "Tadzjikistan"
      ],
      "Tanzania": [
         null,
         "Tanzania"
      ],
      "Thailand": [
         null,
         "Thailand"
      ],
      "Timor-Leste": [
         null,
         "Oost-Timor"
      ],
      "Togo": [
         null,
         "Togo"
      ],
      "Tokelau": [
         null,
         "Tokelau"
      ],
      "Tonga": [
         null,
         "Tonga"
      ],
      "Trinidad and Tobago": [
         null,
         "Trinidad en Tobago"
      ],
      "Tunisia": [
         null,
         "Tunesië"
      ],
      "Turkey": [
         null,
         "Turkije"
      ],
      "Turkmenistan": [
         null,
         "Turkmenistan"
      ],
      "Turks and Caicos Islands": [
         null,
         "Turks- en Caicoseilanden"
      ],
      "Tuvalu": [
         null,
         "Tuvalu"
      ],
      "Uganda": [
         null,
         "Oeganda"
      ],
      "Ukraine": [
         null,
         "Oekraïne"
      ],
      "the United Arab Emirates": [
         null,
         "de Verenigde Arabische Emiraten"
      ],
      "the United Kingdom": [
         null,
         "het Verenigd Koninkrijk"
      ],
      "the United States": [
         null,
         "de Verenigde Staten"
      ],
      "the United States Minor Outlying Islands": [
         null,
         "de Kleine afgelegen eilanden van de Verenigde Staten"
      ],
      "Uruguay": [
         null,
         "Uruguay"
      ],
      "Uzbekistan": [
         null,
         "Oezbekistan"
      ],
      "Vanuatu": [
         null,
         "Vanuatu"
      ],
      "Venezuela": [
         null,
         "Venezuela"
      ],
      "Viet Nam": [
         null,
         "Vietnam"
      ],
      "Virgin Islands (British)": [
         null,
         "Maagdeneilanden (Brits)"
      ],
      "Virgin Islands (U.S.)": [
         null,
         "Maagdeneilanden (Amerikaans)"
      ],
      "Wallis and Futuna": [
         null,
         "Wallis en Futuna"
      ],
      "Western Sahara": [
         null,
         "Westelijke Sahara"
      ],
      "Yemen": [
         null,
         "Jemen"
      ],
      "Zambia": [
         null,
         "Zambia"
      ],
      "Zimbabwe": [
         null,
         "Zimbabwe"
      ],
      "Russian Rubles": [
         null,
         "Russische Roebels"
      ],
      "Indian Rupee": [
         null,
         ""
      ],
      "Brazilian Reals": [
         null,
         ""
      ],
      "Japanese Yen": [
         null,
         ""
      ],
      "Australian Dollars": [
         null,
         ""
      ],
      "Swiss Francs": [
         null,
         "Zwitserse Franken"
      ],
      "Euros": [
         null,
         "Euro"
      ],
      "US Dollars": [
         null,
         "Amerikaanse dollars"
      ],
      "Join now": [
         null,
         "Word nu lid"
      ],
      "Great": [
         null,
         "Goed"
      ],
      "Based on %s reviews": [
         null,
         "Op basis van %s beoordelingen"
      ],
      "Read full review": [
         null,
         "Lees de volledige beoordeling"
      ],
      "Trusted By Companies Including:": [
         null,
         "Vertrouwd door bedrijven zoals:"
      ],
      "World-class VPN service:": [
         null,
         "VPN-service van wereldklasse:"
      ],
      "Protect your online privacy and identity": [
         null,
         "Bescherm uw online privacy en identiteit"
      ],
      "Browse anonymously with a private IP address and enable strong encryption to protect yourself from your ISP and public Wi-Fi networks.": [
         null,
         "Surf anoniem met een privé IP-adres en schakel sterke codering in om uzelf te beschermen tegen uw ISP en openbare wifinetwerken."
      ],
      "Unblock Streaming": [
         null,
         "Deblokkeer streaming"
      ],
      "Get access to tons of new content on platforms like Netflix, Disney Plus, Amazon Prime Video, HBO GO, and more.": [
         null,
         "Krijg toegang tot enorm veel nieuwe inhoud op platforms zoals Netflix, Disney Plus, Amazon Prime Video, HBO GO en meer."
      ],
      "The World's Leading VPN": [
         null,
         "‘s Werelds meest toonaangevende VPN"
      ],
      "Private Internet Access provides the highest speeds and most encryption. So, get a perfect streaming experience without any buffering or slow loading times.": [
         null,
         "Private Internet Access biedt de hoogste snelheden en de beste codering. U krijgt dus een perfecte streamingervaring zonder buffering of trage laadtijden."
      ],
      "Monthly": [
         null,
         "Maandelijks"
      ],
      "Months": [
         null,
         "Maanden"
      ],
      "Yearly": [
         null,
         "Jaarlijks"
      ],
      "Years": [
         null,
         "Jaren"
      ],
      "Best value": [
         null,
         "Beste waarde"
      ],
      "Save <b>%s%</b>": [
         null,
         "Bespaar <b>%s%</b>"
      ],
      "/mo": [
         null,
         "/m"
      ],
      "<del>%s</del> %s per month": [
         null,
         "<del>%s</del> %s per maand"
      ],
      "<del>%s</del> %s per %s months": [
         null,
         "<del>%s</del> %s per %s maanden"
      ],
      "<del>%s</del> %s per year": [
         null,
         "<del>%s</del> %s per jaar"
      ],
      "<del>%s</del>%s per %s years": [
         null,
         "<del>%s</del>%s per %s jaren"
      ],
      "Sign up now": [
         null,
         "Meld u nu aan"
      ],
      "Best<br>value": [
         null,
         "Beste<br>waarde"
      ],
      "+%s extra<br>months free": [
         null,
         "+%s extra<br>maanden gratis"
      ],
      "All amounts are shown in %s, and any discounts reflect a reduction based on the current monthly service pricing at %s per month": [
         null,
         "Alle bedragen worden weergegeven in %s en eventuele kortingen zijn een verlaging van de huidige prijs van %s per maand voor de service"
      ],
      "Why choose Private Internet Access?": [
         null,
         "Waarom Private Internet Access kiezen?"
      ],
      "Total Privacy": [
         null,
         "Totale privacy"
      ],
      "Private Internet Access never keeps any records of users' online activities. You can rest assured knowing you have complete privacy, and nobody knows what you're doing online.": [
         null,
         "Private Internet Access houdt nooit gegevens bij over de online activiteiten van gebruikers. U kunt erop vertrouwen dat u weet dat u volledige privacy hebt en dat niemand weet wat u online doet."
      ],
      "Easy to use": [
         null,
         "Gebruiksvriendelijk"
      ],
      "No matter your technical skills Private Internet Access is one of the easiest VPN apps out there. All it takes to connect is just 1 click or tap and your data will be encrypted instantly.": [
         null,
         "Hoe goed of slecht uw technische vaardigheden ook zijn, Private Internet Access is een van de eenvoudigste VPN-apps die er zijn. U kunt al met 1 muisklik of tik op het scherm verbinding maken en uw gegevens worden onmiddellijk gecodeerd."
      ],
      "Secure VPN Protocols": [
         null,
         "Veilige VPN-protocollen"
      ],
      "Our app is powered by the latest and most advanced protocols like: Wireguard, PPTP, OpenVPN and L2TP/IPSec.": [
         null,
         "Onze app wordt aangedreven door de nieuwste en meest geavanceerde protocollen, zoals Wireguard, PPTP, OpenVPN en L2TP/IPSec."
      ],
      "%s devices simultaneously": [
         null,
         "%s apparaten tegelijkertijd"
      ],
      "Keep all your devices protected. With just 1 Private Internet Access VPN subscription you can connect on up to %s devices at the same time.": [
         null,
         "Bescherm al uw apparaten. Met slechts 1 Private Internet Access VPN-abonnement kunt u tegelijkertijd verbinding maken met maximaal %s apparaten."
      ],
      "Our readers made it abundantly clear that Private Internet Access should be your first stop for protecting your private browsing data.": [
         null,
         "Onze lezers gaven duidelijk aan dat Private Internet Access de beste manier is om uw persoonlijke browsergegevens te beschermen."
      ],
      "Private Internet Access | Anonymous VPN Service Provider": [
         null,
         "Private Internet Access | Anonieme VPN-serviceprovider"
      ],
      "As recommended by": [
         null,
         "Zoals aanbevolen door"
      ],
      "Limited Time Only! <span class='color_pia_green'>Act Now</span>": [
         null,
         "Slechts beperkt geldig! <span class='color_pia_green'>Schaf nu aan</span>"
      ],
      "%s<br><span>day</span>": [
         null,
         "%s<br><span>dagen</span>"
      ],
      "money back guarantee": [
         null,
         "geld-teruggarantie"
      ],
      "Remember, you're not risking anything.": [
         null,
         "Denk eraan, u riskeert helemaal niets."
      ],
      "If you’re not 100% satisfied with <b>Private Internet Access</b>,<br>you have %s full days to ask for a refund and we’ll give you your money back.": [
         null,
         "Als u niet 100% tevreden bent met <b>Private Internet Access</b>,<br>kunt u binnen %s dagen uw geld terugvragen en dan geven wij u het geld terug."
      ],
      "FREE": [
         null,
         "GRATIS"
      ],
      "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 46.0271 120.4594)' font-size='17px'>EXTRA MONTHS</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>FREE": [
         null,
         "2</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 42.4821 73.5978)' font-size='83px'>+</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 46.0271 120.4594)' font-size='17px'>EXTRA MAANDEN</text><text transform='matrix(0.9722 0.2341 -0.2341 0.9722 39.0927 169.5558)' font-size='53px'>GRATIS"
      ],
      "Your IP:": [
         null,
         "Uw IP:"
      ],
      "Your Location:": [
         null,
         "Uw locatie:"
      ],
      "Your ISP:": [
         null,
         "Uw ISP:"
      ],
      "Your Status:": [
         null,
         "Uw status:"
      ],
      "You are not protected": [
         null,
         "U bent niet beschermd"
      ],
      "<p><b class='text-uppercase'>\"Private Internet Access</b></p><p>out-performs and out-features the competition\"</p>": [
         null,
         ""
      ],
      "Connect to servers from all over the world": [
         null,
         "Maak verbinding met servers van over de hele wereld"
      ],
      "Servers": [
         null,
         "Servers"
      ],
      "Countries": [
         null,
         "Landen"
      ],
      "for ONLY": [
         null,
         "voor SLECHTS"
      ],
      "/month": [
         null,
         "/maand"
      ],
      "Proud Sponsor Of": [
         null,
         "Trotse sponsor van"
      ],
      "Compatible with all popular desktop and mobile operating systems": [
         null,
         "Compatibel met alle populaire besturingssystemen op de desktop en mobiel"
      ],
      "For a Limited Time Only<br>Grab our": [
         null,
         "Slechts beperkt geldig<br>Pak onze"
      ],
      "DISCOUNT": [
         null,
         "KORTING"
      ],
      "Experience": [
         null,
         "Ontdek"
      ],
      "Total Online Freedom": [
         null,
         "Totale online vrijheid"
      ],
      "Bypass censorship and unblock sites and content you don't have access to": [
         null,
         "Omzeil censuur en deblokkeer sites en inhoud waartoe u geen toegang hebt"
      ],
      "Secure Protection on Public Wi-Fi": [
         null,
         "Veilige bescherming op openbare wifi"
      ],
      "Keep your data safe and private while connecting to any poorly secured network": [
         null,
         "Houd uw gegevens beveiligd en privé wanneer u verbinding maakt met een slecht beveiligd netwerk"
      ],
      "Unlimited Bandwidth & Fast Download Speeds": [
         null,
         "Onbeperkte bandbreedte en hoge downloadsnelheden"
      ],
      "Keep your downloads private and experience lightning fast speeds for P2P torrenting": [
         null,
         "Houd uw downloads privé en ervaar razendsnelle snelheden bij het downloaden van P2P-torrents"
      ],
      "Ad blocking": [
         null,
         "Advertenties blokkeren"
      ],
      "With Private Internet Access you can block ads, trackers, and malware": [
         null,
         "Met Private Internet Access blokkeert u advertenties, trackers en malware"
      ],
      "Get Private Internet Access VPN": [
         null,
         "Koop Private Internet Access VPN"
      ],
      "For a Limited Time Only": [
         null,
         "Slechts beperkt geldig"
      ],
      "OOPS!": [
         null,
         "OEPS!"
      ],
      "It looks like the page you were looking for isn't here anymore. Feel free to visit our homepage by clicking the button below.": [
         null,
         "Het lijkt erop dat de pagina waarnaar u zocht er niet meer is. Ga naar onze startpagina door op onderstaande knop te klikken."
      ],
      "Take Me To The Homepage": [
         null,
         "Breng me naar de startpagina"
      ],
      "<small>Discover all the</small><br class=\"\"><b><span>streaming content</span></b><br>you’ve been missing out on": [
         null,
         "<small>Ontdek alle</small><br class=\"\"><b><span>streaminginhoud</span></b><br>die u hebt gemist"
      ],
      "Unblock restricted content libraries easily<br> with Private Internet Access": [
         null,
         "Deblokkeer eenvoudig beperkte inhoudsbibliotheken<br> met Private Internet Access"
      ],
      "Try it risk free! <i class='fas fa-award'></i><span class='text-uppercase'> %s-day money back guarantee</span>": [
         null,
         "Probeer het zonder risico's! <i class='fas fa-award'></i><span class='text-uppercase'> %s dagen geld-teruggarantie</span>"
      ],
      "Try it risk free!<br><span class='text-uppercase'>%s-day money back guarantee</span>": [
         null,
         "Probeer het zonder risico's!<br><span class='text-uppercase'>%s dagen geld-teruggarantie</span>"
      ],
      "Private Internet Access VPN for %s% OFF": [
         null,
         "Private Internet Access VPN met %s% KORTING"
      ],
      "Discover Limitless Streaming Content": [
         null,
         "Ontdek onbeperkte streaminginhoud"
      ],
      "Online streaming services and websites are becoming more popular every day all over the world. But sometimes geo-restrictions can limit the content you have access to. Don’t miss out on video streaming opportunities because of your location. <br><b class='mt-1 d-inline-block'>Sign up for Private Internet Access today!</b>": [
         null,
         "Overal ter wereld worden online streamingservices en websites steeds populairder. Maar soms zijn er geografische beperkingen waardoor u slechts toegang hebt tot een beperkte hoeveelheid inhoud. Loop geen streaminginhoud mis vanwege uw locatie. <br><b class='mt-1 d-inline-block'>Meld u vandaag nog aan voor Private Internet Access!</b>"
      ],
      "Unblock content on:": [
         null,
         "Deblokkeer inhoud op:"
      ],
      "Sign up for Private Internet Access today!": [
         null,
         "Meld u vandaag nog aan voor Private Internet Access!"
      ],
      "A must have for your streaming needs": [
         null,
         "Een must-have voor uw streamingbehoeften"
      ],
      "Unlimited bandwidth for streaming": [
         null,
         "Onbeperkte bandbreedte voor streaming"
      ],
      "Private Internet Access has no logs, never records online activity and does not throttle users.": [
         null,
         "Private Internet Access houdt geen logbestanden bij, registreert online activiteiten nooit en beperkt gebruikers niet."
      ],
      "One-click solution to override geographical restrictions": [
         null,
         "Een oplossing met één muisklik om geografische beperkingen te omzeilen"
      ],
      "Once you have chosen your virtual location, you can connect to it using Private Internet Access with just one click.": [
         null,
         "Nadat u uw virtuele locatie heeft gekozen, maakt u met slechts één muisklik verbinding met deze locatie via Private Internet Access."
      ],
      "Stream on multiple devices": [
         null,
         "Streamen op meerdere apparaten"
      ],
      "One Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your entire family!": [
         null,
         ""
      ],
      "Four easy steps to stream with Private Internet Access": [
         null,
         "Vier eenvoudige stappen om te streamen met Private Internet Access"
      ],
      "Start streaming with Private Internet Access in minutes": [
         null,
         "Begin binnen enkele minuten met streamen via Private Internet Access"
      ],
      "<b>Subscribe</b><br>to Private Internet Access": [
         null,
         "<b>Abonneer</b><br>u op Private Internet Access"
      ],
      "<b>Download</b><br>Private Internet Access": [
         null,
         "<b>Download</b><br>Private Internet Access"
      ],
      "<b>Login</b><br>on the Private Internet Access client with your username and password": [
         null,
         "<b>Log in</b><br>op de Private Internet Access-client met uw gebruikersnaam en wachtwoord"
      ],
      "<b>Turn on</b><br>Private Internet Access and connect to the location of your choice": [
         null,
         "<b>Schakel</b><br>Private Internet Access in en maak verbinding met een locatie naar keuze"
      ],
      "You’re ready to start streaming with <span>Private Internet Access</span>": [
         null,
         "Nu bent u klaar om te streamen met <span>Private Internet Access</span>"
      ],
      "Private Internet Access Streaming FAQ": [
         null,
         "Veelgestelde vragen over streamen met Private Internet Access"
      ],
      "Is it safe to stream with Private Internet Access?": [
         null,
         "Is het veilig om te streamen met Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to stream without Private Internet Access?": [
         null,
         "Ja. Private Internet Access biedt een VPN-service met servers op %s locaties in %s landen, waaronder de Verenigde Staten, het Verenigd Koninkrijk, Japan en meer. Eigenlijk is de vraag: is het veilig om zonder Private Internet Access te streamen?"
      ],
      "Do you have a refund policy?": [
         null,
         "Hebben jullie een restitutiebeleid?"
      ],
      "Private Internet Access has a no questions asked %s day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Private Internet Access biedt %s dagen onvoorwaardelijke geld-teruggarantie. Abonneer u vandaag nog op Private Internet Access en begin direct met streamen!"
      ],
      "Will my IP address be exposed to streaming services or websites?": [
         null,
         "Is mijn IP-adres zichtbaar voor streamingservices of websites?"
      ],
      "By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Streaming services and websites will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Wanneer u Private Internet Access gebruikt, codeert u uw verkeer via onze VPN-tunnel die toegang heeft tot internet vanaf het IP-adres van een van onze servers. Streamingservices en websites zien alleen het IP-adres van Private Internet Access, niet het IP-adres van uw apparaat."
      ],
      "Does Private Internet Access limit the bandwidth that streamers can use?": [
         null,
         "Beperkt Private Internet Access de bandbreedte die streamers kunnen gebruiken?"
      ],
      "Private Internet Access is the premier no log VPN - this means that we don’t even know if you are streaming. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Private Internet Access is de beste VPN zonder logbestanden. Dit betekent dat we niet eens weten of u streamt. Al onze servers bieden onbeperkte bandbreedte en we beperken onze gebruikers nooit. Meld u vandaag nog aan bij Private Internet Access en geniet van onbeperkt en ongecensureerd internet."
      ],
      "Will Private Internet Access be blocked on my favorite streaming service or website?": [
         null,
         "Wordt Private Internet Access geblokkeerd op mijn favoriete streamingservice of website?"
      ],
      " Private Internet Access believes in an open internet and is always working to make sure that no blocks exist for our VPN users. Use our service today to see why Private Internet Access is the best VPN for streaming online.": [
         null,
         "Private Internet Access gelooft in een open internet en doet er alles aan om alle blokkades voor onze VPN-gebruikers weg te nemen. Gebruik onze service vandaag nog en ontdek waarom Private Internet Access de beste VPN is voor online streaming."
      ],
      "Save <b>%s%</b><br><span class=\"lines\">& get <b>%s extra months <span>free</span></b></span>": [
         null,
         "Bespaar <b>%s%</b><br><span class=\"lines\">& krijg <b>%s extra maanden <span>gratis</span></b></span>"
      ],
      "Save <b>%s%</b> & get<br><b class=\"lines\">%s extra months</b><br><span>free</span>": [
         null,
         "Bespaar <b>%s%</b> & krijg<br><b class=\"lines\">%s extra maanden</b><br><span>gratis</span>"
      ],
      "Save <b>%s%</b><br>& get<br><b class=\"lines\">%s extra months</b><br><span>free</span>": [
         null,
         "Bespaar <b>%s%</b><br>& krijg<br><b class=\"lines\">%s extra maanden</b><br><span>gratis</span>"
      ],
      "for only <b class=\"mx-md-3 mx-2\"><span>%s</span><span>%s</span></b> /month": [
         null,
         "voor<br>slechts <b class=\"mx-md-3 mx-2\"><span>%s</span><span>%s</span></b> /maand"
      ],
      "The VPN recommended by<br><b>%s</b>": [
         null,
         "De VPN die wordt aanbevolen door<br><b>%s</b>"
      ],
      "Save %s% Now": [
         null,
         "Bespaar nu %s%"
      ],
      "Sign-up now & get %s extra months <span class='text-uppercase'>free</span>": [
         null,
         "Meld u nu aan en krijg %s extra maanden <span class='text-uppercase'>gratis</span>"
      ],
      "Try Out the World's<br><b>Most <span class='color_pia_light_orange'>Trusted</span><br>& <span class='color_pia_light_orange'>Secure VPN</span></b>": [
         null,
         "Probeer 's werelds<br><b>meest <span class='color_pia_light_orange'>betrouwbare</span><br>en <span class='color_pia_light_orange'>veilige VPN</span></b>"
      ],
      "Try it risk free! <i class='fas fa-award'></i> %s day money back guarantee": [
         null,
         "Probeer het zonder risico’s! <i class='fas fa-award'></i> %s dagen geld-teruggarantie"
      ],
      "Discover The Safest Way to Get<br><b><span class=\"text-uppercase color_pia_light_orange\">Total Online Privacy</span></b><br>Get Private Internet Access VPN": [
         null,
         "Ontdek de veiligste manier voor<br><b><span class=\"text-uppercase color_pia_light_orange\">totale online privacy</span></b><br>Download Private Internet Access VPN"
      ],
      "100%</span> No Logs & Records": [
         null,
         "100%</span> Geen logboeken en records"
      ],
      "Rated as the Best VPN Service In<br><b><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "Beoordeeld als de beste VPN-service in<br><b><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "No Logs": [
         null,
         "Geen logbestanden"
      ],
      "Military-Grade Encryption": [
         null,
         "Codering op militair niveau"
      ],
      "%s Devices": [
         null,
         "%s apparaten"
      ],
      "Using <span>Zoom</span> for video calls?": [
         null,
         "Gebruikt u <span>Zoom</span> voor videogesprekken?"
      ],
      "Make sure your online activity stays hidden": [
         null,
         "Zorg ervoor dat uw online activiteit verborgen blijft"
      ],
      "Why use a VPN with Zoom?": [
         null,
         "Waarom een ​​VPN met Zoom gebruiken?"
      ],
      "Zoom has been accused of sharing user data with Facebook, they lied about the app having end-to-end encryption, and the app allowed meeting hosts to track attendees.<br><br><b>So get Private Internet Access to encrypt your data, hide your IP address and become anonymous while using Zoom.</b>": [
         null,
         "Zoom is beschuldigd van het delen van gebruikersgegevens met Facebook. Daarnaast heeft het bedrijf onterecht beweerd dat Zoom end-to-end-codering bevat en konden hosts hun deelnemers volgen via de app.<br><br><b>Schaf Private Internet Access dus aan om uw gegevens te coderen, uw IP-adres te verbergen en anoniem te blijven terwijl u Zoom gebruikt.</b>"
      ],
      "First-rate VPN service:": [
         null,
         "Eersteklas VPN-service:"
      ],
      "Bypass censorship and unblock streaming content and sites you don't have access to. Discover geo-restricted content on platforms like Netflix, HBO GO, Amazon Prime, Disney Plus, and more.": [
         null,
         "Omzeil censuur en deblokkeer streaminginhoud en -sites waartoe u geen toegang hebt. Ontdek geografisch beperkte inhoud op platforms zoals Netflix, HBO GO, Amazon Prime, Disney Plus en meer."
      ],
      "Lightning-fast Internet": [
         null,
         "Razendsnel internet"
      ],
      "Private Internet Access provides the highest speeds possible. So, you can have calls without any buffering or connectivity issues.": [
         null,
         "Private Internet Access biedt de hoogst mogelijke snelheden. U kunt dus gesprekken voeren zonder buffer- of verbindingsproblemen."
      ],
      "Unbreakable Encryption": [
         null,
         "Onbreekbare codering"
      ],
      "Encrypt your data to make sure nobody can hack your connection and get access to your data.": [
         null,
         "Codeer uw gegevens zodat dat niemand uw verbinding kan hacken en toegang kan krijgen tot uw gegevens."
      ],
      "Become Anonymous": [
         null,
         "Word anoniem"
      ],
      "Browse anonymously with a private IP address and protect yourself from your companies that want to get their hands on your data.": [
         null,
         "Surf anoniem met een privé IP-adres en bescherm uzelf tegen bedrijven die uw gegevens in handen willen krijgen."
      ],
      "Connect in 4 easy steps:": [
         null,
         "Maak verbinding in 4 simpele stappen:"
      ],
      "<b>Turn on</b><br>Private Internet Access and connect": [
         null,
         "<b>Schakel</b><br>Private Internet Access in en maak verbinding"
      ],
      "You’re ready to start using <span>Private Internet Access</span> with zoom": [
         null,
         "Nu bent u klaar om <span>Private Internet Access</span> te gebruiken met Zoom"
      ],
      "Use PIA on up to %s devices simultaneously": [
         null,
         "Gebruik PIA op maximaal %s apparaten tegelijkertijd"
      ],
      "%s-Day money back guarantee": [
         null,
         "%s dagen geld-teruggarantie"
      ],
      "<span class=\"color_pia_green\">Frequently asked questions</span>": [
         null,
         "<span class=\"color_pia_green\">Veelgestelde vragen</span>"
      ],
      "How to use Private Internet Access with Zoom?": [
         null,
         "Hoe gebruik ik Private Internet Access met Zoom?"
      ],
      "First download and install the Private Internet Access app on the device you're planning to use for your Zoom call. Then create and verify your account. Sing-in to the PIA app using your username and password and connect to a server by clicking or tapping the big, round power button. You're now completely anonymous and protected, all that's left to do is to fire-up your Zoom app and join your conference.": [
         null,
         "Download en installeer eerst de Private Internet Access-app op het apparaat dat u wilt gebruiken voor uw Zoom-gesprek. Maak en verifieer vervolgens uw account. Meld u aan in de PIA-app met uw gebruikersnaam en wachtwoord en maak verbinding met een server door op de grote ronde aan/uit-knop te klikken of tikken. U bent nu volledig anoniem en beschermd. U hoeft alleen nog maar de Zoom-app te starten en deel te nemen aan uw vergadering."
      ],
      "Private Internet Access has a no questions asked %s-day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Private Internet Access biedt %s dagen onvoorwaardelijke geld-teruggarantie. Abonneer u vandaag nog op Private Internet Access en begin direct met streamen!"
      ],
      "Is it safe to use Private Internet Access?": [
         null,
         "Is het veilig om Private Internet Access te gebruiken?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. Private Internet access never records any of its users' data. We follow a strict no-logs policy that ensures we never keep any logs of our users' activities online.": [
         null,
         "Ja. Private Internet Access is een VPN-service met servers in %s locaties in %s landen, waaronder de Verenigde Staten, het Verenigd Koninkrijk, Japan en meer. Private Internet Access registreert nooit gegevens van gebruikers. We volgen een strikt Geen logbestanden-beleid, wat betekent dat we nooit logbestanden bewaren van de online activiteiten van onze gebruikers."
      ],
      "Will my IP address be exposed while using Zoom?": [
         null,
         "Is mijn IP-adres zichtbaar wanneer ik Zoom gebruik?"
      ],
      "No. By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Streaming services, apps and websites will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Nee. Wanneer u Private Internet Access gebruikt, codeert u uw verkeer via onze VPN-tunnel die toegang heeft tot internet vanaf het IP-adres van een van onze servers. Streamingservices, apps en websites zien alleen het IP-adres van Private Internet Access, niet het IP-adres van uw apparaat."
      ],
      "Get Unbreakable Online Security With<br><b>The Best VPN for<br><span class=\"color_pia_light_orange\">%s</span></b>": [
         null,
         "IJzersterke online beveiliging met<br><b>de beste VPN voor<br><span class=\"color_pia_light_orange\">%s</span></b>"
      ],
      "Unblock <span>new streaming content</span><br><small>on your <span>Fire Stick TV</span></small>": [
         null,
         "Deblokkeer <span>nieuwe streaminginhoud</span><br><small>op uw <span>Fire TV Stick</span></small>"
      ],
      "with Private Internet Access VPN": [
         null,
         "met Private Internet Access VPN"
      ],
      "Tired of watching the same shows and movies on streaming platforms?": [
         null,
         "Wilt u niet steeds dezelfde series en films bekijken op streamingplatforms?"
      ],
      "Private Internet Access changes your IP address to one from a different location so you can discover new streaming properties.<br><b>Sign up to Private Internet Access today and unblock tons of new content from different countries around the world.</b>": [
         null,
         "Private Internet Access verandert uw IP-adres in een adres op een andere locatie, zodat u nieuwe streaminginhoud kunt ontdekken.<br><b>Meld u vandaag nog aan voor Private Internet Access en deblokkeer een scala aan nieuwe inhoud uit verschillende landen over de hele wereld.</b>"
      ],
      "Stream new content on your Fire Stick TV:": [
         null,
         "Stream nieuwe inhoud op uw Fire TV Stick:"
      ],
      "A must have for your Fire Stick TV": [
         null,
         "Een must-have voor uw Fire TV Stick"
      ],
      "Access content from all over the world": [
         null,
         "Toegang tot inhoud van over de hele wereld"
      ],
      "Private Internet Access has thousands of servers in over %s countries world-wide. Connect to either one of them and discover new films, TV shows and documentaries.": [
         null,
         "Private Internet Access heeft duizenden servers in meer dan %s landen over de hele wereld. Maak verbinding met een van deze servers en ontdek nieuwe films, tv-series en documentaires."
      ],
      "Override geographical restrictions": [
         null,
         "Omzeil geografische beperkingen"
      ],
      "Connect to a server from a different country with just one click, straight from the Private Internet Access app, and unblock content that's not available in your country.": [
         null,
         "Maak met slechts één klik verbinding met een server uit een ander land, direct via de Private Internet Access-app, en deblokkeer inhoud die niet beschikbaar is in uw land."
      ],
      "With just 1 Private Internet Access VPN subscription you can use the app on up to %s devices at the same time. This way you can unblock content on anything that has a screen and an internet connection.": [
         null,
         "Met slechts 1 abonnement op Private Internet Access VPN kunt u de app tegelijkertijd op maximaal %s apparaten gebruiken. Zo kunt u inhoud deblokkeren op alle apparaten met een scherm en een internetverbinding."
      ],
      "All it takes are 4 easy steps:": [
         null,
         "Er zijn slechts 4 simpele stappen:"
      ],
      "Watch your favorite movies and shows in minutes": [
         null,
         "Bekijk binnen enkele minuten uw favoriete films en series"
      ],
      "Got questions about streaming? <span class=\"color_pia_green\">Here are a few of our FAQs</span>": [
         null,
         "Hebt u vragen over streaming? <span class=\"color_pia_green\">Dit zijn enkele veelgestelde vragen</span>"
      ],
      "Private Internet Access is the premier no log VPN - this means that we don’t even know if you are streaming. All our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Private Internet Access is de beste VPN zonder logbestanden. Dit betekent dat we niet eens weten of u streamt. Al onze servers bieden onbeperkte bandbreedte en we beperken onze gebruikers nooit. Meld u vandaag nog aan bij Private Internet Access en geniet van onbeperkt en ongecensureerd internet."
      ],
      "Private Internet Access believes in an open internet and is always working to make sure that no blocks exist for our VPN users. Use our service today to see why Private Internet Access is the best VPN for streaming online.": [
         null,
         "Private Internet Access gelooft in een open internet en doet er alles aan om alle blokkades voor onze VPN-gebruikers weg te nemen. Gebruik onze service vandaag nog en ontdek waarom Private Internet Access de beste VPN is voor online streaming."
      ],
      "Discover an endless amount of <br><b>streaming content</b> on <span>KODI</span>": [
         null,
         "Ontdek een eindeloze hoeveelheid <br><b>streaminginhoud</b> op <span>KODI</span>"
      ],
      "Access geo-blocked TV shows and movies <br>while staying anonymous online": [
         null,
         "Toegang tot geografisch geblokkeerde tv-programma's <br>en films terwijl u online anoniem blijft"
      ],
      "Don’t get into trouble while streaming on Kodi": [
         null,
         "Kom niet in de problemen tijdens het streamen op Kodi"
      ],
      "Whether you use Kodi TV to watch your personal content library or for the titles you find on their website, make sure nobody will be able to find out what you're doing online.<br><b>Why? </b>Streaming content on Kodi works is basically P2P torrenting which might be illegal in your country. Luckily, with the <b>Private Internet Access VPN</b>, you can make all your internet activity totally anonymous. So, you'll never have any trouble with governmental authorities while using Kodi.": [
         null,
         "Of u Kodi TV nu gebruikt om inhoud uit uw persoonlijke bibliotheek te bekijken of voor de titels op hun website, zorg ervoor dat niemand kan achterhalen wat u online doet.<br><b>Waarom? </b>Het streamen van inhoud op Kodi is in feite hetzelfde als het downloaden van P2P-torrents, en dit is mogelijk illegaal in uw land. Gelukkig blijven al uw internetactiviteiten volledig anoniem met de <b>Private Internet Access VPN</b>. U komt dus nooit in de problemen met overheidsinstanties terwijl u Kodi gebruikt."
      ],
      "P2P traffic allowed": [
         null,
         "P2P-verkeer toegestaan"
      ],
      "Private Internet Access supports P2P protocols such as torrents.": [
         null,
         "Private Internet Access ondersteunt P2P-protocollen zoals torrents."
      ],
      "No Logs Policy": [
         null,
         "Geen logbestanden-beleid"
      ],
      "Private Internet Access never records its users' online activity and never keeps any logs, so you'll have complete anonymity from the moment you connect to our servers.": [
         null,
         "Private Internet Access registreert nooit de online activiteiten van gebruikers en houdt nooit logbestanden bij, dus u bent volledig anoniem vanaf het moment dat u verbinding maakt met onze servers."
      ],
      "Bypass geographical restrictions": [
         null,
         "Omzeil geografische beperkingen"
      ],
      "Once you have chosen the location you want to use the internet from, Private Internet Access will always be one click away.": [
         null,
         "Zodra u hebt gekozen op welke locatie u internet wilt gebruiken, gebruikt u Private Internet Access al met één muisklik."
      ],
      "Use Kodi TV on multiple devices": [
         null,
         "Gebruik Kodi TV op meerdere apparaten"
      ],
      "One Private Internet Access subscription can be used on up to %s devices simultaneously. Share Private Internet Access with your whole family!": [
         null,
         ""
      ],
      "Watch Kodi anonymously in 4 easy steps": [
         null,
         "Kijk Kodi anoniem in 4 eenvoudige stappen"
      ],
      "You’re ready to start using <span>Private Internet Access</span> with your Kodi TV": [
         null,
         "Nu bent u klaar om <span>Private Internet Access</span> te gebruiken met uw Kodi TV"
      ],
      "Got questions? <span class=\"color_pia_green\">Check out our FAQs</span>": [
         null,
         "Hebt u vragen? <span class=\"color_pia_green\">Bekijk onze veelgestelde vragen</span>"
      ],
      "Is it safe to use Kodi with Private Internet Access?": [
         null,
         "Is het veilig om Kodi te gebruiken met Private Internet Access?"
      ],
      "Yes. Private Internet Access provides a VPN service with servers in %s locations across %s countries, including the United States, United Kingdom, Japan, and more. The better question is: Is it safe to use Kodi without Private Internet Access?": [
         null,
         "Ja. Private Internet Access biedt een VPN-service met servers op %s locaties in %s landen, waaronder de Verenigde Staten, het Verenigd Koninkrijk, Japan en meer. Eigenlijk is de vraag: is het veilig om Kodi te gebruiken zonder Private Internet Access?"
      ],
      "Yes. Private Internet Access has a no questions asked %s-day money back guarantee. Subscribe to Private Internet Access today to start streaming now!": [
         null,
         "Ja. Private Internet Access biedt %s dagen onvoorwaardelijke geld-teruggarantie. Abonneer u vandaag nog op Private Internet Access en begin direct met streamen!"
      ],
      "Will my IP address be exposed while using Kodi add-ons?": [
         null,
         "Is mijn IP-adres zichtbaar wanneer ik Kodi add-ons gebruik?"
      ],
      "No. By using Private Internet Access, you are encrypting your traffic through our VPN tunnel which accesses the internet from one of our servers’ IP addresses. Any service or website accessed will only see our Private Internet Access IP address, not your device’s IP address.": [
         null,
         "Nee. Wanneer u Private Internet Access gebruikt, codeert u uw verkeer via onze VPN-tunnel die toegang heeft tot internet vanaf het IP-adres van een van onze servers. Alle geopende services en websites zien alleen het IP-adres van Private Internet Access, niet het IP-adres van uw apparaat."
      ],
      "Does Private Internet Access limit bandwidth for Kodi users?": [
         null,
         "Beperkt Private Internet Access de bandbreedte voor Kodi-gebruikers?"
      ],
      "No. Private Internet Access is the premier no log VPN - this means that we don’t even know what software you use in conjunction with ours. All of our servers offer unlimited bandwidth and we do not throttle our users. Period. Sign up for Private Internet Access today to enjoy the unlimited and uncensored internet.": [
         null,
         "Nee. Private Internet Access is de beste VPN zonder logbestanden. Dit betekent dat we niet eens weten welke software u samen met onze software gebruikt. Al onze servers bieden onbeperkte bandbreedte en we beperken onze gebruikers nooit. Meld u vandaag nog aan bij Private Internet Access en geniet van onbeperkt en ongecensureerd internet."
      ],
      "Experience the Best Way<br><b>to Stay <span class='color_pia_light_orange'><span class='text-uppercase'>Safe</span> on Public Wi-Fi</span><br>& <span class='color_pia_light_orange text-uppercase'>Never Risk</span> Getting Hacked </b>": [
         null,
         "Ontdek de beste manier<br><b>om <span class='color_pia_light_orange'><span class='text-uppercase'>beveiligd te blijven</span> op openbare wifi</span><br>en <span class='color_pia_light_orange text-uppercase'>vermijd het risico</span> dat u gehackt wordt </b>"
      ]
   }
}
;
